...
$rptQuery2="SELECT accounts.user_id as user_id, accounts.account_num, accounts.company, labs.lab_name, orders.order_num as order_num, orders.po_num, orders.order_total, 
orders.order_date_processed, orders.order_date_shipped, orders.order_patient_first, orders.order_product_name,orders.redo_reason_id, orders.order_patient_last, orders.tray_num,
accounts.company, orders.order_status, orders.redo_order_num, orders.prescript_lab from orders, accounts, labs
WHERE 
accounts.user_id = orders.user_ID 
AND redo_order_num IS NOT NULL
AND prescript_lab NOT IN ( 10, 3,4,73,2 )
AND order_status NOT IN ('cancelled','filled','on hold')
GROUP BY order_num";
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
	...
	if($Debug == 'yes')
	echo '<br>queryManufacturer: <br>'. $queryManufacturer . '<br>';
	}
	...
	if($Debug == 'yes')
	echo '<br>queryRedoReason: <br>'. $queryRedoReason . '<br>';
	...
	if ($listItem[redo_order_num] <> ''){
		...
		if($Debug == 'yes')
		echo '<br>queryLab: <br>'. $queryLab . '<br>';
...
$to_address	    = array('dbeaulieu@direct-lens.com','kgawel@direct-lens.com','dbeaulieu@direct-lens.com','abedard@entrepotdelalunette.com');
//$to_address	    = array('dbeaulieu@direct-lens.com');
$curTime        = date("m-d-Y");	
$from_address   = 'donotreply@entrepotdelalunette.com';
$subject		= "HBC Redos waiting for redirection: ".$curTime;
//SEND EMAIL
if ($SendEmail == 'yes'){
