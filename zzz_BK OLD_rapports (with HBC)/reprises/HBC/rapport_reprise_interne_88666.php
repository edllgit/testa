...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	if($Debug == 'yes')
	echo '<br>queryEstShipDate: <br>'. $queryEstShipDate . '<br>';	
	...
	if($Debug == 'yes')
	echo '<br>queryredo: <br>'. $queryredo . '<br>';	
...
$queryRedoExterne = "SELECT order_num, user_id, tray_num, order_date_processed, order_product_name, order_patient_first, 
order_patient_last, patient_ref_num, order_status, prescript_lab,  redo_order_num
FROM orders
...
if($Debug == 'yes')
echo '<br>queryRedoExterne: <br>'. $queryRedoExterne . '<br>';	
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Old Account:88666%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Internal Redos Store Griffé: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
