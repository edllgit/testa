...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('88414') AND order_num = $RedoOrderNum";
...
FROM orders
WHERE user_id IN ('88414')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Old Account:88414%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Internal Redos Store #88414-Yorkdale: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
