...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('montreal','montrealsafe') AND order_num = $RedoOrderNum";
	...
	if (($DataRedo[user_id] == 'montreal') || ($DataRedo[user_id] == 'montrealsafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('montreal')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:montreal%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Redo Interne Entrepot de le lunette Montreal HBC Zone Tendance 1: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport reprises interne Montreal ZT1 2.0', '$time','$today','$heure_execution','rapport_reprise_interne_mtl-zt1.php')"  ; 					
