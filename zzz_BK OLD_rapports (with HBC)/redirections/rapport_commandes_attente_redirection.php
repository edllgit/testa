...
$rptResult=	$conn->query($rptQuery2);	//or die  ('I cannot select items because 1: ' . mysqli_error($con). '<br>'. $rptQuery2);
...
switch($listItem["user_id"]){//N'inclue pas les compte SAFE
	...
	if ($listItem[prescript_lab] <> ''){
		...
		$resultManufacturer = $conn->query($queryManufacturer);
	...
	$resultRedoReason = $conn->query($queryRedoReason);
...
$to_address	    = array('dbeaulieu@direct-lens.com','dbeaulieu@direct-lens.com');
...
$cronResult		 = $conn->query($CronQuery);
