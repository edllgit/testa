...
$rptQuery="SELECT  orders.prescript_lab, orders.order_num, orders.order_product_name, accounts.main_lab, accounts.company, orders.order_date_processed, orders.order_item_date,  orders.user_id, orders.po_num, orders.order_total, orders.order_date_processed, orders.order_date_shipped, orders.order_patient_first, orders.order_patient_last, orders.tray_num,  orders.order_status, orders.internal_note from orders, accounts
	WHERE  orders.user_id not in ('eyelationnet','eyelationcan') AND lab <> 37 and Lab <> 47 AND accounts.user_id = orders.user_id  and orders.order_status IN ('order imported',			
'job started',	'interlab',	'in coating','in mounting',	'in edging','delay issue 1','delay issue 2', 'delay issue 3','delay issue 4','delay issue 5','delay issue 6',	'waiting for frame', 'waiting for frame swiss','waiting for frame knr','waiting for shape','re-do','verifying',		
'order completed','on hold','scanned shape to swiss','') AND order_num<>-1 AND orders.lab NOT IN (26,37) order by order_status, order_date_processed";
...
VALUES('Rapport Late Jobs 2.0', '$time','$today','$timeplus3heures','rapport_late_jobs.php','$ips') "  ; 					
