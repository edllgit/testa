	...
	function calc_current_balance($user_id) {
		...
		if($rptCount) {
			while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
				...
				$result=mysqli_query($con,"SELECT DATE_ADD('$listItem[order_date_processed]', interval 1 month)");/* add 1 month to order_processed_date for due date */
				//$duedate=mysql_result($result,0,0);
				$result=mysqli_query($con,"SELECT DATE_SUB('$duedate', interval 15 day)");/* add 15 days to order_processed_date */
				//$discountdate_15=mysqli_result($result,0,0);
				$result=mysqli_query($con,"SELECT DATE_SUB('$duedate', interval 10 day)");/* add 10 days to order_processed_date */
		...
		$memoResult=mysqli_query($con,$memoQuery) or die  ('I cannot select memo credits because: ' . mysqli_error($con).$memoQuery);
		$memoCount=mysqli_num_rows($memoResult);
		if($memoCount > 0){
			$memoCredTotal=0;
			while ($memoData=mysqli_fetch_array($memoResult,MYSQLI_ASSOC)){
