...
<input name="mypkey" type="hidden" id="mypkey" value="<?php print $_SESSION['PrescrData']['mypkey'];?>" />
              <tr >
				...
				if ($stock_quantity>0)
					...
					$resultOrderTotal    = mysqli_query($con,$queryOrderTotal)		or die  ('I cannot select items 2 because: ' . mysqli_error($con));
					$countResult         = mysqli_num_rows($resultOrderTotal);
					$StockOrderValue = 0;
					if ($countResult  > 0){
						while ($DataOrderTotal = mysqli_fetch_array($resultOrderTotal,MYSQLI_ASSOC)){
