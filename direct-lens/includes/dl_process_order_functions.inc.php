...
//FIN PROMO LENSNET TRANSITIONS
	...
	/*
	//Code pour Summer Si$$le! Promotion
	$querySelectedPromo  = "SELECT selected_promotion FROM accounts WHERE user_id = '". $_SESSION["sessionUser_Id"]. "'";
	$resultSelectedPromo = mysql_query($querySelectedPromo) or die  ('I cannot select items because: ' . mysql_error());
...
$DebutNomProduit = substr($listItem[order_product_name],0,5);
	...
	//if ($DataSelectedPromo[selected_promotion]=='sizzling summer'){
		//echo '<br>Produit:'. $listItem[order_product_name]; 
		//echo '<br>$DebutNomProduit: ' . $DebutNomProduit;
		//echo '<br><br>Coupon dsc:' . $listItem[coupon_dsc];
		//echo '<br><br>selected promo:' . $DataSelectedPromo[selected_promotion];
		//echo '<br><br>Order type:' . $listItem[order_product_type];
		//if ($DebutNomProduit =='Promo')
		//echo '<br>CONTIENT PROMO NE PAS EMETTRE DE ROYAUTE';	
	//}
	
	
	if (($listItem[coupon_dsc]=='0.00') && ($DataSelectedPromo[selected_promotion]=='sizzling summer') && ($listItem[order_product_type]=='exclusive')  && ($DebutNomProduit <>'Promo')) {
	
	//echo '<br><br>TOUT EST OK POUR EMETTRE DES ROYALTIES';
	
	$Royaute = 0;
	
	$queryLensCategory   = "SELECT lens_category FROM exclusive WHERE  primary_key = ".$listItem[order_product_id];
	$resultLensCategory  = mysql_query($queryLensCategory) or die  ('I cannot select items because: ' . mysql_error());
	...
	//1- On �value si le produit command� est du lens_Category prog cl, Si oui, on ajoute 2,50$ 
	if ($LensCategory =='prog cl')
	$Royaute = $Royaute + 2.5;
	
	//2- On �value si le produit command� est du lens_Category prog ds ou prog ff, Si oui, on ajoute 5$ 
	if (($LensCategory =='prog ds') || ($LensCategory =='prog ff'))
	$Royaute = $Royaute + 5;
	
	//3- On �value si le coating est  parmis ces coatings:/*Dream AR, Smart AR,ITO AR,Xlr,MultiClear AR,Aqua Dream AR,CrizalF,Blue AR, sI UN DE CES COATING + 2.5$
   ...
   switch($listItem[order_product_coating])
   {
	...
	//4- On �value si le produit a un transition si oui,  + 2.5$
	 //echo '<br>Photo: ' .$listItem[order_product_photo];
   ...
   //4- On �value si le produit a un polarized si oui,  + 2.5$
...
$the_order_num  = $orderNum;
$salesperson_id = $listItem[salesperson_id];
$royalty_amount = $Royaute;
$lens_category  = $LensCategory;
$coating 	    = $listItem[order_product_coating];
$polar          = $listItem[order_product_polar];
$transition     = $listItem[order_product_photo];

//Ins�rer un tuple dans  table royalty	
$queryRoyalty = "INSERT INTO royalty (user_id, product_name,order_num,salesperson_id,royalty_amount,royalty_date, lens_category, coating, polar, photo) 
VALUES ('$user_id', '$product_name','$the_order_num','$salesperson_id','$royalty_amount','$order_date_processed','$lens_category','$coating','$polar','$transition')";
$resultInsert=mysql_query($queryRoyalty)		or die  ('Please contact our support. The following error occured: ' . mysql_error());
		
//echo '<br><br>'. $queryRoyalty;
}
//echo '<br><br>Total des royalty: '.  $Royaute;

	
	}//End if aucun coupon appliqu� sur commande ET que le client a choisir la promo Summer Si$$le!
	*/
