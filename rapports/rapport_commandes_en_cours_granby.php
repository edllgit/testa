...
include('../ses_email_functions.inc.php');
...
FROM orders, labs
WHERE orders.prescript_lab = labs.primary_key
AND user_id IN ('granby','granbysafe')
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){ 			
	...
	if ($list_order_status == 'Reprise Interne'){	
		...
		$resultRepriseInterne = mysqli_query($con,$queryRepriseInterne)		or die  ('I cannot select items because 4: ' . mysqli_error($con));
	...
	if ($nbrResult > 0)	
	{
		...
		$resultEstShiPDate    = mysqli_query($con,$queryEstShipDate)		or die  ('I cannot select items because 5: ' . mysqli_error($con));
		$DataEstShipDate      = mysqli_fetch_array($resultEstShiPDate,MYSQLI_ASSOC);	
		$EstimateShipDate     = $DataEstShipDate[est_ship_date];
	}
		...
		switch($DataRepriseInterne["order_status"]){		
			...
			case 'interlab qc':	    	$list_order_statusRedo = "Interlab QC";   			break;
	...
	if ($Redo_Order_Num <> ''){	
		...
		$resultLastUpdate   = mysqli_query($con,$queryLastUpdate)		or die  ('I cannot select items because 6:' . $queryLastUpdate . mysqli_error($con));
		$DataLastUpdate     = mysqli_fetch_array($resultLastUpdate,MYSQLI_ASSOC); 	
		$StatusLastUpdate   = substr($DataLastUpdate[update_time],0,10);
	}//End IF
		$message.="
...
$to_address = array('rapports@direct-lens.com','granby@entrepotdelalunette.com');
//$to_address   = array('mdesilets@entrepotdelalunette.com');


$curTime      = date("m-d-Y");	
$from_address = 'donotreply@direct-lens.com';
$subject      = "Entrepot de le lunette Granby: commandes en cours";

//SEND EMAIL
	if ($SendEmail == 'yes'){
		$response = ses_mail($to_address, $from_address, $subject, null, $message);
	}
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_en_cours_granby'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/granby/' . $nomFichier . '.html';
		file_put_contents($cheminFichierHtml, $message);
	
		echo 'Rapport sauvegardé au format HTML : ' . $cheminFichierHtml . '<br>';
	
	if($response){ 
		//log_email("REPORT: Entrepot de le lunette Granby: commandes en cours",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
		echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully to:'. $EmailEnvoyerA.'</strong></div>';
	}else{
		//log_email("REPORT: Entrepot de le lunette Granby: commandes en cours",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
...
VALUES('Entrepot lunette Granby  Orders In production 2.0', '$time','$today','$timeplus3heures','.php') "  ; 					
