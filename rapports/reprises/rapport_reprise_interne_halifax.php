...
FROM orders
WHERE user_id IN ('St.Catharines','redosafety','St. Catharines')
...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('warehousehal','warehousehalsafe') AND order_num = $RedoOrderNum";
	...
	$StatusLastUpdate   = substr($DataLastUpdate[update_time],0,10);		
	 ...
	 if (($DataRedo[user_id] == 'warehousehal') || ($DataRedo[user_id] == 'warehousehalsafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('warehousehal')   AND order_status NOT IN ('cancelled','on hold')
...
$subject      = "Internal Redo Optical Warehouse Halifax: $today";  
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_halifax'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Quotidien_&_Others/halifax/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport reprises interne Halifax 2.0', '$time','$today','$heure_execution','rapport_reprise_interne_halifax.php')"; 					
