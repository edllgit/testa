...
FROM orders
WHERE user_id IN ('granby','granbysafe')   AND order_status NOT IN ('cancelled','on hold')
...
$subject      = "Redo Interne Mensuel Entrepot de la lunette Granby: $AnneeEnCours-$JourDebut  - $AnneeEnCours-$JourFin";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_mensuel_granby'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Mensuel/granby/' . $nomFichier . '.html';
