...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('chicoutimi','chicoutimisafe') AND order_num = $RedoOrderNum";
	...
	if (($DataRedo[user_id] == 'chicoutimi') || ($DataRedo[user_id] == 'chicoutimisafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('chicoutimi')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:chicoutimi%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Redo Interne Entrepot de le lunette Chicoutimi: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_chicoutimi'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Quotidien_&_Others/chicoutimi/' . $nomFichier . '.html';
