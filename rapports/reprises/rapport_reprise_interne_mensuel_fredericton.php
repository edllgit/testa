...
FROM orders
WHERE user_id IN ('fredericton','frederictonsafe')   AND order_status NOT IN ('cancelled','on hold')
...
$subject      = "Redo Interne Mensuel Entrepot de la lunette Fredericton: $AnneeEnCours-$JourDebut  - $AnneeEnCours-$JourFin";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_mensuel_fredericton'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Mensuel/Fredericton/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport reprises interne Fredericton', '$time','$today','$heure_execution','rapport_reprise_interne_mensuel_fredericton.php')"; 					
