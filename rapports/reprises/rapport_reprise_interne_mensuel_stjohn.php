...
FROM orders
WHERE user_id IN ('stjohn','stjohnsafe')   AND order_status NOT IN ('cancelled','on hold')
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_mensuel_stjohn'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Mensuel/stjohn/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport reprises interne St-John', '$time','$today','$heure_execution','rapport_reprise_interne_mensuel_stjohn.php')"; 					
