...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('entrepotifc','entrepotsafe') AND order_num = $RedoOrderNum";
	...
	if (($DataRedo[user_id] == 'entrepotifc') || ($DataRedo[user_id] == 'entrepotsafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('entrepotifc')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:entrepotifc%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Redo Interne Entrepot de le lunette Trois-Rivieres: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_trois-rivieres'. $timestamp;
