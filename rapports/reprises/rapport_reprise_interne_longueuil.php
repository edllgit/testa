...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('longueuil','longueuilsafe') AND order_num = $RedoOrderNum";
	...
	if (($DataRedo[user_id] == 'longueuil') || ($DataRedo[user_id] == 'longueuilsafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('longueuil')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:longueuil%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Redo Interne Entrepot de le lunette Longueuil: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_longueuil'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Quotidien_&_Others/longueuil/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page)
					VALUES('Rapport reprises interne Longueuil 2.0', '$time','$today','$heure_execution','rapport_reprise_interne_longueuil.php')"; 					
