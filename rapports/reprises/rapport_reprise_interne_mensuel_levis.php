...
FROM orders
WHERE user_id IN ('levis','levissafe')   AND order_status NOT IN ('cancelled','on hold')
...
$subject      = "Redo Interne Mensuel Entrepot de la lunette Lévis: $AnneeEnCours-$JourDebut  - $AnneeEnCours-$JourFin";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_mensuel_levis'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Mensuel/levis/' . $nomFichier . '.html';
