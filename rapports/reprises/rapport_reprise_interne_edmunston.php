...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('edmundston','edmundstonsafe') AND order_num = $RedoOrderNum";
	...
	if (($DataRedo[user_id] == 'edmundston') || ($DataRedo[user_id] == 'edmundstonsafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('edmundston')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:edmundston%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$subject      = "Redo Interne Entrepot de le lunette Edmundston: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_edmundston'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Quotidien_&_Others/edmundston/' . $nomFichier . '.html';
