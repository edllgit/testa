...
FROM orders
WHERE user_id IN ('entrepotquebec','quebecsafe')   AND order_status NOT IN ('cancelled','on hold')
...
$subject      = "Redo Interne Mensuel Entrepot de la lunette Québec: $AnneeEnCours-$JourDebut  - $AnneeEnCours-$JourFin";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_mensuel_quebec'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Mensuel/quebec/' . $nomFichier . '.html';
