...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:entrepotdr%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$message.="
   ...
   <tr bgcolor=\"CCCCCC\">
	...
	$resultEstShiPDateGTC  = mysqli_query($con,$queryEstShipDateGTC)		or die  ('I cannot select items because 2 ' . $queryEstShipDateGTC . mysqli_error($con));
...
$subject      = "Redo Interne Entrepot de le lunette Drummondville: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_drummondville'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Quotidien_&_Others/drummondville/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport reprises interne Drummondville 2.0', '$time','$today','$timeplus3heures','rapport_reprise_interne_drummondville.php')"; 					
