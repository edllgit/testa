...
$RedoOrderNum = $listItem[redo_order_num];		
	...
	FROM orders 
	WHERE user_id IN ('dartmouth','dartmouthsafe') AND order_num = $RedoOrderNum";
	...
	if (($DataRedo[user_id] == 'dartmouth') || ($DataRedo[user_id] == 'dartmouthsafe'))
	{
	$message.="
	<tr>
...
FROM orders
WHERE user_id IN ('dartmouth')   AND order_status NOT IN ('cancelled','on hold')
...
FROM orders
WHERE user_id IN ('garantieatoutcasser') AND internal_note like '%Ancien compte:dartmouth%'
AND order_date_processed = '$today'
ORDER BY  order_date_processed";
//echo '<br>'.$queryGTC .'<br>';
...
$message.="</table>";
$to_address = array('rapports@direct-lens.com','dartmouth@opticalwarehouse.ca');



//$to_address = array('rapports@direct-lens.com');
$from_address = 'donotreply@entrepotdelalunette.com';
$subject      = "Redo Interne Entrepot de le lunette Dartmouth: $today";
//SEND EMAIL
	if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_reprise_interne_dartmouth'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/REPRISE/Quotidien_&_Others/dartmouth/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport reprises interne Dartmouth 2.0', '$time','$today','$heure_execution','rapport_reprise_interne_dartmouth.php')"; 					
