...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){ 			
	...
	$resultEstShiPDate  = mysqli_query($con,$queryEstShipDate)		or die  ('I cannot select items because 2a: ' . mysqli_error($con));
	...
	$resultLastUpdate   = mysqli_query($con,$queryLastUpdate)		or die  ('I cannot select items because 2b: ' . mysqli_error($con));
...
$message.="</table>";
$to_address = array('rapports@direct-lens.com','quebec@entrepotdelalunette.com');


//$to_address = array('rapports@direct-lens.com');
$curTime	  = date("m-d-Y");	
$from_address ='donotreply@entrepotdelalunette.com';
$subject      = "Entrepot de le lunette Quebec: commandes en cours";
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_en_cours_quebec'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/quebec/' . $nomFichier . '.html';
	...
	if($response){ 
		//log_email("REPORT: Entrepot de le lunette Quebec: commandes en cours",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
		echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully to:'. $EmailEnvoyerA.'</strong></div>';
	}else{
		//log_email("REPORT: Entrepot de le lunette Quebec: commandes en cours",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
...
VALUES('Rapport commandes en cours Quebec 2.0', '$time','$today','$timeplus3heures','rapport_commandes_en_cours_quebec.php')"; 					
