...
WHERE   lab NOT IN (66,67,59,37) AND accounts.user_id = orders.user_id  and orders.order_status IN ('order imported',			
'job started',	'interlab',	'in coating', 'in mounting',	'in edging','delay issue 1','delay issue 2','delay issue 3','delay issue 4',
'delay issue 5','delay issue 6'	,	'waiting for frame','waiting for frame swiss','waiting for shape'	,'re-do','verifying','in transit','order completed','on hold','scanned shape to swiss','') 
AND order_num<>-1 ORDER BY order_status asc, order_date_processed";
...
//SEND EMAIL
$send_to_address = array('rapports@direct-lens.com','kgawel@direct-lens.com','jmotyka@direct-lens.com');	
...
$subject= "EDLL Late orders (NO EDLL)";
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_late_jobs_jen_'. $timestamp;
...
VALUES('Rapport Late Jobs Jen 2.0', '$time','$today','$timeplus3heures','rapport_late_jobs_jen.php','$ips') "  ; 					
