...
FROM orders, labs
WHERE orders.prescript_lab = labs.primary_key
AND user_id IN ('chicoutimi','chicoutimisafe')
...
$subject      = "Entrepot de le lunette Chicoutimi: commandes en cours";
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_en_cours_chicou'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/chicoutimi/' . $nomFichier . '.html';
	...
	if($response){ 
		//log_email("REPORT: Entrepot de le lunette Chicoutimi: commandes en cours",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
		echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully to:'. $EmailEnvoyerA.'</strong></div>';
	}else{
		//log_email("REPORT: Entrepot de le lunette Chicoutimi: commandes en cours",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
...
VALUES('Entrepot lunette Chicoutimi Orders In production 2.0', '$time','$today','$timeplus3heures','cron_entrepot_lunette_chicoutimi_daily_report.php') "  ; 					
