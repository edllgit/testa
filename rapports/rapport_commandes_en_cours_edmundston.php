...
FROM orders, labs
WHERE orders.prescript_lab = labs.primary_key
AND user_id IN ('edmundston','edmundstonsafe')
...
$subject      = "Entrepot de le lunette Edmundston: commandes en cours";
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_en_cours_edmundston'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/edmundston/' . $nomFichier . '.html';
	...
	if($response){ 
		//log_email("REPORT: Entrepot de le lunette edmundston: commandes en cours",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
		echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully to:'. $EmailEnvoyerA.'</strong></div>';
	}else{
		//log_email("REPORT: Entrepot de le lunette edmundston: commandes en cours",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
...
VALUES('Entrepot lunette Edmundston Orders In production 2.0', '$time','$today','$timeplus3heures','rapport_commandes_en_cours_edmundston.php')"; 					
