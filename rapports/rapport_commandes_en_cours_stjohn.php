...
FROM orders, labs
WHERE orders.prescript_lab = labs.primary_key
AND user_id IN ('stjohn','stjohnsafe')
...
$subject      = "Entrepot de le lunette stjohn: commandes en cours";
...
//Log email
	...
	$nomFichier = 'r_commande_en_cours_stjohn'. $timestamp;

	// Enregistrez le contenu HTML dans un fichier
	$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/stjohn/' . $nomFichier . '.html';
	...
	if($response){ 
		//log_email("REPORT: Entrepot de le lunette stjohn: commandes en cours",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
		echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully to:'. $EmailEnvoyerA.'</strong></div>';
	}else{
		//log_email("REPORT: Entrepot de le lunette stjohn: commandes en cours",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
...
VALUES('Entrepot lunette stjohn Orders In production 2.0', '$time','$today','$timeplus3heures','rapport_commandes_en_cours_stjohn.php')"; 					
