...
'warehousestcsafe','lavalsafe','warehousehalsafe','terrebonnesafe','quebecsafe','sherbrookesafe','chicoutimisafe')
    AND (orders.order_status!='cancelled' AND orders.order_status!='basket')
    GROUP BY order_num";	
	break;
...
if ($ordersnum == 0)
echo '<div class="alert alert-warning" role="alert"><strong>0 order redirected to GKB, no report generated</strong></div>';
...
if ($ordersnum!=0){
	...
	$subject      = "Orders of the day EDLL - GKB: $today ";
	...
	$nomFichier = 'r_redirection_gkb_edll_'. $timestamp;
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport redirection GKB Edll', '$time','$today','$heure_execution','rapport_redirection_gkb_edll.php')"; 					
