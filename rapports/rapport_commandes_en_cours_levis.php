...
AND user_id IN ('levis','levissafe')
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){ 			
	...
	if ($nbrResult > 0)	
	{
		...
		switch($DataRepriseInterne["order_status"]){		
			...
			default:  				    $list_order_statusRedo = 'UNKNOWN';	                break;					
		}
...
$message.="</table>";
$to_address = array('rapports@direct-lens.com','levis@entrepotdelalunette.com');
...
$subject      = "Entrepot de le lunette Levis: commandes en cours";
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_en_cours_levis'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/levis/' . $nomFichier . '.html';
...
$CronQuery       = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport commandes en cours Levis 2.0', '$time','$today','$timeplus3heures','rapport_commandes_en_cours_levis.php') "  ; 					
