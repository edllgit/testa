...
WHERE  accounts.user_id = orders.user_id  and orders.order_status ='waiting for lens'
...
$rptResult = mysqli_query($con,$rptQuery)		or die  ('I cannot select items because: <br><br>' . mysql_error($con));
...
$subject	  = 'Liste des commandes waiting for lens:'. $curTime;
if ($SendEmail == 'yes'){
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_waiting_for_lens_'. $timestamp;
...
$CronQuery 		 = "INSERT INTO cron_duration(cron_script_name,cron_duration, cron_date, cron_time,php_page) 
					VALUES('Rapport Commandes Attente de Verre 2.0', '$time','$today','$timeplus3heures','rapport_commandes_waiting_lens.php')"; 					
