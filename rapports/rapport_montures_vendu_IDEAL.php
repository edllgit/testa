...
$ordersnum=mysqli_num_rows($rptResult);
		...
		$queryFrame = "SELECT orders.order_num, extra_product_orders.frame_type, extra_product_orders.supplier, extra_product_orders.model, extra_product_orders.temple_model_num, extra_product_orders.color
		, extra_product_orders.ep_frame_a, extra_product_orders.ep_frame_dbl, orders.code_source_monture FROM extra_product_orders JOIN orders ON orders.order_num = extra_product_orders.order_num WHERE order_num = $listItem[order_num]
		and  temple_model_num LIKE '%FOCUS%' AND category IN ('Frame', 'Edging') AND code_source_monture LIKE '%V1%' ";
