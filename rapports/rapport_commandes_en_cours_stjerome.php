...
$rptQuery   = "SELECT order_num, user_id, tray_num, order_date_processed, order_product_name, order_patient_first, order_patient_last, patient_ref_num, order_status, prescript_lab, lab_name, redo_order_num
FROM orders, labs
WHERE orders.prescript_lab = labs.primary_key
AND user_id IN ('stjerome') 
...
FROM orders, labs
WHERE orders.prescript_lab = labs.primary_key
AND user_id IN ('stjerome','stjeromesafe')
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){ 			
	...
	if ($nbrResult > 0)	
	{
		...
		switch($DataRepriseInterne["order_status"]){		
			...
			case 'scanned shape to swiss': 	$list_order_statusRedo = "Scanned shape to Swiss"; 		break;
			case 'scanned shape to swiss': 	$list_order_statusRedo = "Scanned shape to Swiss"; 		break;			
	...
	if ($IlyaUnRedoInterne == 'oui'){//S'il y a un 2ieme redo interne pour cette commande
		...
		if ($nbrResult2iemeredo  > 0){//If there is a second redo
			...
			switch($Data2iemeRedo["order_status"]){		
				...
				case 'scanned shape to swiss': 	$status2iemeredo = "Scanned shape to Swiss"; 		break;
...
$subject      = "Entrepot de la lunette St-Jérôme: commandes en cours";
...
//Log email
	...
	foreach($to_address as $key => $value)
	{
		...
		$nomFichier = 'r_commande_en_cours_stJerome'. $timestamp;
	
		// Enregistrez le contenu HTML dans un fichier
		$cheminFichierHtml = 'C:/All_Rapports_EDLL/general/Commande/en_cours/stjerome/' . $nomFichier . '.html';
