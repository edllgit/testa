...
<b>1.5% charge on invoice not paid within 30 days following the date of statement
	...
	//Send a copy to the Tracy
	$CopyAdmin = "tkenyeres@direct-lens.com";
	$to_address = str_split($CopyAdmin,100);
	$from_address='donotreply@entrepotdelalunette.com';
	$curTime= date("m-d-Y");	
	$response=office365_mail($to_address, $from_address, $subject, null, $message);
...
$query="SELECT * from orders WHERE user_id='$user_id' and order_num='$orderNum' and order_product_type='exclusive' ORDER by order_item_date";//SELECT ALL OPEN PRESCRIPTION ORDERS
			...
			if ($usercount != 0){
						...
						if ($e_listItem[category]=="Edging"){
							...
							if ($e_listItem[category]=="Prism"){
								if ($Product_line=='eye-recommend'){
									...
									$resultPrisme = mysql_query($queryPrisme) or die  ('I cannot select Prism items because: ' . mysql_error().$queryPrisme);
									$DataPrisme   = mysql_fetch_array($resultPrisme);
					...
					$resultAlreadyPaid  = mysql_query($queryAlreadyPaid) or die ("Could not find order num");
					$nombreResultat     = mysql_num_rows($resultAlreadyPaid);
					
					if ($nombreResultat > 0 ){
						$DataAlreadyPaid = mysql_fetch_array($resultAlreadyPaid);
...
$sendPrices = true;
	...
	elseif ($listItem['product_line']=="milano6769") 
		{$dl_logo_file="http://direct-lens.com/logos/logoblanc.png";
...
$message .= '</head>';
	...
	if ($dl_logo_file<>"http://direct-lens.com/logos/logoblanc.png")
         ...
         <tr >
              ...
              <tr>
				...
				$message.='</b></td>
					...
					$resultTotaltoPay = mysql_query($queryTotaltoPay)		or die  ('I cannot select items because 9: ' . mysql_error());
					$TotalaPayer = 0;
					while ($DataTotaltoPay=mysql_fetch_array($resultTotaltoPay)){
...
$resultTotalQty  = mysql_query($queryTotalQty)		or die  ('I cannot select items because 8: ' . mysql_error());
$DataProductline = mysql_fetch_array($resultTotalQty);
