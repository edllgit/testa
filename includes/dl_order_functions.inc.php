...
$MainLabResult=mysqli_query($con,$MainLabQuery)	or die  ('I cannot select items because: ' . mysqli_error($con));
$MainLabData=mysqli_fetch_array($MainLabResult,MYSQLI_ASSOC);
				...
				if ($oneyear_dt >=$datecomplete)
				{
					...
					if ($oneyear_ar_credit > 0)
					{
						...
						$availableForPromo2ndPair = 'yes';
							...
							$result=mysqli_query($con,$queryUpdateRebate) or die ( "Query failed: " . mysqli_error($con) . "<br/>" . $queryUpdateRebate );
							...
							$resultCounter=mysqli_query($con,$queryUpdateCounter) or die ( "Query failed: " . mysqli_error($con) . "<br/>" . $queryUpdateCounter );
...
$_SESSION['PrescrData']['SPECIAL_INSTRUCTIONS']=mysqli_real_escape_string($con,strtoupper($_POST[SPECIAL_INSTRUCTIONS]));
...
//$Position = strpos($_SESSION['PrescrData']['SPECIAL_INSTRUCTIONS'], 'BASE'); 
	...
	/*$_SESSION['PrescrData']['INTERNAL_NOTE']=mysqli_real_escape_string($con,strtoupper($_POST[INTERNAL_NOTE]));*/
	//}
...
function applyCouponCode($pkey,$coupon_code){//APPLY COUPON CODE TO ORDER TABLE AND COUPON USE TABLE
	...
	$result=mysqli_query($con,$query) or die ('Could not update because B2' . mysqli_error($con));
	...
	$result=mysqli_query($con,$query) or die ('Could not update because D4: ' . mysqli_error($con));
...
function removeCouponCode($pkey){//APPLY COUPON CODE TO ORDER TABLE AND COUPON USE TABLE
	...
	$result=mysqli_query($con,$query) or die ('Could not update because C3: ' . $query.'<br>' . mysqli_error($con));
