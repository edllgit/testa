<?php
	function calc_current_balance($user_id) {
		...
		if($rptCount) {
			while ($listItem=mysql_fetch_array($rptResult)){
				...
				$orderTotal=bcadd($listItem[order_total], $listItem[order_shipping_cost], 2);//add shipping cost
					...
					print "<br>CLOSED=".$listItem[order_num]."   ".$discounted_total_cost;
					
				if($listItem[pmt_amount]==0){//this order has NOT been paid
					$pmt_status="Open";
					$pmt_amount="";
					print "<br>OPEN=".$listItem[order_num]."   ".$discounted_total_cost;
					...
					print "<br>PARTIAL=".$listItem[order_num]."   ".$discounted_total_cost;
						if($order_subtotal > 0){
		...
		print "<BR>TOTAL ORDERS:".$grandTotal;
