...
<table width='$WidthTableau' border=\"1\" align=\"center\" cellpadding=\"3\" cellspacing=\"0\" class=\"formField\">
  ...
  // $Report_Email	= array('fdjibrilla@entrepotdelalunette.com','dbeaulieu@direct-lens.com','gbruneau@entrepotdelalunette.com','dbeaulieu@direct-lens.com','dbeaulieu@direct-lens.com','thahn@direct-lens.com');
	$Report_Email	= array('fdjibrilla@entrepotdelalunette.com');
...
//$Report_Email= array('fdjibrilla@entrepotdelalunette.com','gbruneau@entrepotdelalunette.com','lbouthillier@entrepotdelalunette.com','fdjibrilla@entrepotdelalunette.com');//LIVE
...
function CalculerIncentiveEDLL($Userid_Magasin,$Description_Magasin,$date1,$date2){
	...
	*/
	...
	$message.= "
		<tr>
			<th align=\"center\">&nbsp;</th>
			<th bgcolor=\"#e0c1b3\" align=\"center\" colspan=\"3\">PROGRESSIVE</th>
	...
	$message.= "
		<tr>
			...
			<th bgcolor=\"#e0c1b3\" align=\"center\"><br>Progressif Advance</th>
			
			<!--th bgcolor=\"#e0c1b3\" align=\"center\"><br> Ifree PLUS Advance</th -->
			
			<th bgcolor=\"#e0c1b3\" align=\"center\"><br>Maxiwide</th>
			<th bgcolor=\"#e0c1b3\" align=\"center\"><br>Precision AI</th>
	...
	while ($DataDistinctEmployees = mysqli_fetch_array($resultDistinctEmployees,MYSQLI_ASSOC)){
		...
		*/
			...
			//Bonus AD 
			//HORS PROMO SEULEMENT: [Prgogressif Advance]
			$Description_Bonus_AD  	= "Prgogressif Advance(10$/job)"; //Description de ce qui donne droit au bonus
			$ValeurBonus_AD	 	    = 10;							//Définir la valeur de ce bonus: x$/Commande
			$user_id_AD 	 		 	= $Userid_Magasin;				//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_AD 	= "  (order_product_name like '%Progressif advance%')"; //Nom de produit à utiliser pour le Filtre		
			$Coating_A_Filtrer_AD_DUO	 	=  " 1=1 "; 
			
			//La requête	
			$queryIncentive_AD 	= "SELECT count(order_num) as Nbr_Bonus_Atteint_AD FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_AD
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled')
			AND (order_product_name not LIKE '%alpha hd%' AND order_product_name not like '%4k%')  
			OR
			redo_order_num IS NULL 
			AND user_id IN $user_id_AD
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled')
			AND (order_product_name not LIKE '%ultimate%') 
			OR
			redo_order_num IS NULL 
			AND user_id IN $user_id_AD
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			...
			AND (order_product_name LIKE '%Precision 1%') 
			ORDER BY order_product_name";
			echo $queryIncentive_AD .'<br><br>';
			
			$resultIncentive_AD 	= mysqli_query($con, $queryIncentive_AD)	or die  ('<br><br>I cannot select items because 1a: <br><br>'. $queryIncentive_AD . mysqli_error($con));
			$DataIncentive_AD 	= mysqli_fetch_array($resultIncentive_AD,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_AD = $DataIncentive_AD[Nbr_Bonus_Atteint_AD];		
			//Calcul du bonus total mérité  par CET Employé 
			$ResultatValeurBonusCourrant_AD = $ValeurBonus_AD  * $Nbr_Bonus_Atteint_AD;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_AD;
			//Fin partie Bonus B HORS PROMO
			
			//PROMO SEULEMENT: [Progressif Adance]
			$Description_Bonus_AD_DUO  	= "Progressif Advance (1$/job)"; //Description de ce qui donne droit au bonus
			$ValeurBonus_AD_DUO 		 	= 10;							//Définir la valeur de ce bonus: x$/Commande
			$user_id_AD_DUO 	 		 	= $Userid_Magasin;				//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_AD_DUO 	= " (order_product_name like '%Progressif advance%')  "; //Nom de produit à utiliser pour le Filtre	
			$Coating_A_Filtrer_AD_DUO	 	=  " 1=1 "; 					//Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_AD_DUO 	= "SELECT count(order_num) as Nbr_Bonus_Atteint_AD_DUO FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_AD
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_AD_DUO
			AND  $Coating_A_Filtrer_AD_DUO
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";			
			$resultIncentive_AD_DUO 	= mysqli_query($con, $queryIncentive_AD_DUO)	or die  ('<br><br>I cannot select items because 1b: <br><br>'. $queryIncentive_AD_DUO . mysqli_error($con));
			$DataIncentive_AD_DUO 	= mysqli_fetch_array($resultIncentive_AD_DUO,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_AD_DUO = $DataIncentive_AD_DUO[Nbr_Bonus_Atteint_AD_DUO];		
			//Calcul du bonus total mérité  par CET Employé 
			$ResultatValeurBonusCourrant_AD_DUO = $ValeurBonus_AD_DUO  * $Nbr_Bonus_Atteint_AD_DUO;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_AD_DUO;
			//Fin partie Bonus B HORS PROMO
			//Fin Bonus B
			...
			//Bonus A: 
			//HORS PROMO SEULEMENT: [Ifree Plus Advance]
			
			/*
			$Description_Bonus_pAD  	= "ifree PLus Advance (10$/job)"; //Description de ce qui donne droit au bonus
			$ValeurBonus_pAD 		 	= 10;					//Définir la valeur de ce bonus: x$/Commande
			$user_id_pAD	 		 	= $Userid_Magasin;		//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_pAD 	= " (order_product_name like '%ifree plus advance%') "; //Nom de produit à utiliser pour le Filtre			
			$Coating_A_Filtrer_pAD 	= " 1=1 "; //Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_pAD 	= "SELECT count(order_num) as Nbr_Bonus_Atteint_pAD FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_pAD
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_pAD
			AND  $Coating_A_Filtrer_pAD 
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			
			echo '<br> $queryIncentive_pAD:'. $queryIncentive_pAD.'<br>';
			
			$resultIncentive_pAD 	= mysqli_query($con, $queryIncentive_pAD)	or die  ('<br><br>I cannot select items because 1c: <br><br>'. $queryIncentive_pAD . mysqli_error($con));
			$DataIncentive_pAD 	= mysqli_fetch_array($resultIncentive_pAD,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_pAD = $DataIncentive_pAD[Nbr_Bonus_Atteint_pAD];		
			//Calcul du bonus total mérité par CET Employé 
			$ResultatValeurBonusCourrant_pAD = $ValeurBonus_pAD  * $Nbr_Bonus_Atteint_pAD;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_pAD;
			//Fin partie Bonus A HORS PROMO		
			

			//PROMO SEULEMENT: [Ifree Plus Advance]
			$Description_Bonus_pAD_DUO  	= "Ifree Plus Advance (1$/job)"; //Description de ce qui donne droit au bonus
			$ValeurBonus_pAD_DUO 		 	= 0.5;							//Définir la valeur de ce bonus: x$/Commande
			$user_id_pAD_DUO 	 		 	= $Userid_Magasin;				//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_pAD_DUO 	= " (order_product_name like '%ifree plus advance%')  "; //Nom de produit à utiliser pour le Filtre	
			$Coating_A_Filtrer_pAD_DUO	 	=  " 1=1 "; 					//Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_pAD_DUO 	= "SELECT count(order_num) as Nbr_Bonus_Atteint_pAD_DUO FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_pAD
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_pAD_DUO
			AND  $Coating_A_Filtrer_pAD_DUO
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";			
			$resultIncentive_pAD_DUO 	= mysqli_query($con, $queryIncentive_pAD_DUO)	or die  ('<br><br>I cannot select items because 1d: <br><br>'. $queryIncentive_pAD_DUO . mysqli_error($con));
			$DataIncentive_pAD_DUO 	= mysqli_fetch_array($resultIncentive_pAD_DUO,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_pAD_DUO = $DataIncentive_pAD_DUO[Nbr_Bonus_Atteint_pAD_DUO];		
			//Calcul du bonus total mérité  par CET Employé 
			$ResultatValeurBonusCourrant_pAD_DUO = $ValeurBonus_pAD_DUO  * $Nbr_Bonus_Atteint_pAD_DUO;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_pAD_DUO;
			//Fin partie Bonus B HORS PROMO
			//Fin Bonus B
			...
			*/
			...
			//Bonus C: 
			//HORS PROMO SEULEMENT: [Precision AI]
			$Description_Bonus_AI  	= "Precision AI (10$/job)"; //Description de ce qui donne droit au bonus
			$ValeurBonus_AI 		 	= 10;					//Définir la valeur de ce bonus: x$/Commande
			$user_id_AI	 		 	= $Userid_Magasin;		//Définir le nom d'utilisateur du magasin 
			
			$NomProduitPourBonus_AI 	= " (order_product_name like '%Precision AI%')  "; //Nom de produit à utiliser pour le Filtre	
			
			$Coating_A_Filtrer_AI 	= " 1=1 "; //Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_AI 	= "SELECT count(order_num) as Nbr_Bonus_AtteintC FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_AI
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_AI
			AND  $Coating_A_Filtrer_AI 
			AND order_product_name not like '%duo%'
			AND order_product_name not like '%promo%'
			AND order_product_name not like '%SV%'
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentive_AI 	= mysqli_query($con, $queryIncentive_AI)	or die  ('<br><br>I cannot select items because 1e: <br><br>'. $queryIncentive_AI . mysqli_error($con));
			$DataIncentive_AI 	= mysqli_fetch_array($resultIncentive_AI,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_AI = $DataIncentive_AI[Nbr_Bonus_Atteint_AI];		
			//Calcul du bonus total mérité par CET Employé 
			$ResultatValeurBonusCourrant_AI = $ValeurBonus_AI  * $Nbr_Bonus_Atteint_AI;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_AI;
			//Fin partie Bonus C HORS PROMO		
			
			//PROMO SEULEMENT: [Precision AI]
			$Description_Bonus_AI_DUO  	= "Precision AI (10$/job)"; //Description de ce qui donne droit au bonus
			$ValeurBonus_AI_DUO 		 	= 10;					//Définir la valeur de ce bonus: x$/Commande
			$user_id_AI_DUO	 		 	= $Userid_Magasin;		//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_AI_DUO 	= " (order_product_name like '%Precision AI%') "; //Nom de produit à utiliser pour le Filtre	
			$Coating_A_Filtrer_AI_DUO 	= " 1=1 "; //Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_AI_DUO 	= "SELECT count(order_num) as Nbr_Bonus_Atteint_AI_DUO FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_AI
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_AI_DUO
			AND  $Coating_A_Filtrer_AI_DUO 
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name"; 
			$resultIncentive_AI_DUO 	= mysqli_query($con, $queryIncentive_AI_DUO)	or die  ('<br><br>I cannot select items because 1f: <br><br>'. $queryIncentive_AI_DUO . mysqli_error($con));
			$DataIncentive_AI_DUO 	= mysqli_fetch_array($resultIncentive_AI_DUO,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_AI_DUO = $DataIncentive_AI_DUO[Nbr_Bonus_Atteint_AI_DUO];		
			//Calcul du bonus total mérité par CET Employé 
			$ResultatValeurBonusCourrant_AI_DUO = $ValeurBonus_AI_DUO  * $Nbr_Bonus_Atteint_AI_DUO;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_AI_DUO;
			//Fin partie Bonus C PROMO	SEULEMENT
			//FIN BONUS C
		
		
			//Bonus D: 
			//HORS PROMO SEULEMENT:[Maxiwide]
			$Description_Bonus_M  	= "Maxiwide (10$/job)"; 	//Description de ce qui donne droit au bonus
			$ValeurBonus_M  		 	= 10;					//Définir la valeur de ce bonus: x$/Commande
			$user_id_M 	 		 	= $Userid_Magasin;		//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_M  	= "order_product_name like '%maxiwide%'"; //Nom de produit à utiliser pour le Filtre	
			$Coating_A_Filtrer_M  	= " 1=1 "; //Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_M  	= "SELECT count(order_num) as Nbr_Bonus_Atteint_M FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_M 
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_M
			AND  $Coating_A_Filtrer_M
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentive_M  	= mysqli_query($con, $queryIncentive_M)	or die  ('<br><br>I cannot select items because 1g: <br><br>'. $queryIncentiveD . mysqli_error($con));
			$DataIncentive_M  	= mysqli_fetch_array($resultIncentive_M ,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_M  = $DataIncentive_M [Nbr_Bonus_Atteint_M];		
			//Calcul du bonus total mérité par CET Employé 
			$ResultatValeurBonusCourrant_M = $ValeurBonus_M  * $Nbr_Bonus_Atteint_M;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_M;
			//Fin partie Promo F HORS PROMO SEULEMENT

			//PROMO SEULEMENT:[Maxiwide]
			$Description_Bonus_M_DUO  	= "Maxiwide (3$/job)"; 	//Description de ce qui donne droit au bonus
			$ValeurBonus_M_DUO 		 	= 10;					//Définir la valeur de ce bonus: x$/Commande
			$user_id_M_DUO	 		 	= $Userid_Magasin;		//Définir le nom d'utilisateur du magasin 
			$NomProduitPourBonus_M_DUO 	= "order_product_name like '%maxiwide%' AND (order_product_name like '%duo%' OR order_product_name like '%promo%') "; //Nom de produit à utiliser pour le Filtre	
			$Coating_A_Filtrer_M_DUO 	= " 1=1 "; //Nom de coating à utiliser pour le Filtre		
			//La requête	
			$queryIncentive_M_DUO 	= "SELECT count(order_num) as Nbr_Bonus_Atteint_M_DUO FROM orders
			WHERE redo_order_num IS NULL 
			AND user_id IN $user_id_M 
			AND salesperson_id='$DataDistinctEmployees[salesperson_id]'	
			AND $NomProduitPourBonus_M_DUO
			AND  $Coating_A_Filtrer_M_DUO
			AND order_date_processed BETWEEN '$date1' AND '$date2'
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentive_M_DUO 	= mysqli_query($con, $queryIncentive_M_DUO)	or die  ('<br><br>I cannot select items because 1h: <br><br>'. $queryIncentive_M_DUO . mysqli_error($con));
			$DataIncentive_M_DUO 	= mysqli_fetch_array($resultIncentive_M_DUO,MYSQLI_ASSOC);
			$Nbr_Bonus_Atteint_M_DUO = $DataIncentive_M_DUO[Nbr_Bonus_Atteint_M_DUO];		
			//Calcul du bonus total mérité par CET Employé 
			$ResultatValeurBonusCourrant_M_DUO = $ValeurBonus_M_DUO  * $Nbr_Bonus_Atteint_M_DUO;	
			//Ajouter au cummul par employé
			$TotalIncentiveForthisEmployee+= $ResultatValeurBonusCourrant_M_DUO;
		...
		/*
			...
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentiveF 	= mysqli_query($con, $queryIncentiveF)	or die  ('<br><br>I cannot select items because 1i: <br><br>'. $queryIncentiveF . mysqli_error($con));
			...
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentiveF_DUO 	= mysqli_query($con, $queryIncentiveF_DUO)	or die  ('<br><br>I cannot select items because 1j: <br><br>'. $queryIncentiveF_DUO . mysqli_error($con));
			...
			AND order_product_name NOT like '%duo%'
			ORDER BY order_product_name";
			$resultIncentiveG 	= mysqli_query($con, $queryIncentiveG)	or die  ('<br><br>I cannot select items because 1k: <br><br>'. $queryIncentiveG . mysqli_error($con));
			...
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentiveG_DUO 	= mysqli_query($con, $queryIncentiveG_DUO)	or die  ('<br><br>I cannot select items because 1l: <br><br>'. $queryIncentiveG_DUO . mysqli_error($con));
			...
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentiveH 	= mysqli_query($con, $queryIncentiveH)	or die  ('<br><br>I cannot select items because 1m: <br><br>'. $queryIncentiveH . mysqli_error($con));
			...
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentiveI 	= mysqli_query($con, $queryIncentiveI)	or die  ('<br><br>I cannot select items because 1n: <br><br>'. $queryIncentiveI . mysqli_error($con));
			...
			AND order_status NOT IN ('on hold', 'cancelled') 	
			ORDER BY order_product_name";
			$resultIncentiveI_DUO 	= mysqli_query($con, $queryIncentiveI_DUO)	or die  ('<br><br>I cannot select items because 1o: <br><br>'. $queryIncentiveI_DUO . mysqli_error($con));
		...
		*/
		...
		$TotalBonus_AD = $ResultatValeurBonusCourrant_AD + $ResultatValeurBonusCourrant_AD_DUO;
		//[BETTER]:Precision Advance
		$message.= 		"<td bgcolor=\"#e0c1b3\" align=\"center\"><b>Régulier</b>: ". $Nbr_Bonus_Atteint_AD . " x $ValeurBonus_AD$ ="."$ResultatValeurBonusCourrant_AD$
		<br> TOTAL:$TotalBonus_AD$				
		</td>";
		//<b>Promo</b>: "		. $Nbr_Bonus_Atteint_AD_DUO . " x $ValeurBonus_AD_DUO$ ="."$ResultatValeurBonusCourrant_AD_DUO$	<br>	
		
		/*
		$TotalBonus_pAD = $ResultatValeurBonusCourrant_pAD + $ResultatValeurBonusCourrant_pAD_DUO;
		//[BEST]:ifree plus advance(Pas encore créé)
		$message.= 		"<td bgcolor=\"#e0c1b3\" align=\"center\"><b>Régulier</b>: ". $Nbr_Bonus_Atteint_pAD . " x $ValeurBonus_pAD$ ="."$ResultatValeurBonusCourrant_pAD$
		<br> <b>Promo</b>: "		. $Nbr_Bonus_Atteint_pAD_DUO . " x $ValeurBonus_pAD_DUO$ ="."$ResultatValeurBonusCourrant_pAD_DUO$		 <br>TOTAL:$TotalBonus_pAD$					
		</td>";
		*/
		
					
		$TotalBonus_AI = $ResultatValeurBonusCourrant_AI + $ResultatValeurBonusCourrant_AI_DUO;
		//[PREMIUM]:Precision AI
		$message.= 		"<td bgcolor=\"#e0c1b3\" align=\"center\"><b>Régulier</b>: ". $Nbr_Bonus_Atteint_AI . " x $ValeurBonus_AI$ ="."$ResultatValeurBonusCourrant_AI$
		<br> TOTAL:$TotalBonus_AI$				
		</td>";
		//<b>Promo</b>: "		. $Nbr_Bonus_Atteint_AI_DUO . " x $ValeurBonus_AI_DUO$ ="."$ResultatValeurBonusCourrant_AI_DUO$				 <br>
		
		$TotalBonus_M = $ResultatValeurBonusCourrant_M + $ResultatValeurBonusCourrant_M_DUO;		
		//[OTHER=OFFICE]:Maxiwide
		$message.= 		"<td bgcolor=\"#e0c1b3\" align=\"center\"><b>Régulier</b>: ". $Nbr_Bonus_Atteint_M . " x $ValeurBonus_M$ ="."$ResultatValeurBonusCourrant_M$
		<br>TOTAL:$TotalBonus_M$		
		</td>";
		// <b>Promo</b>: "		. $Nbr_Bonus_Atteint_M_DUO . " x $ValeurBonus_M_DUO$ ="."$ResultatValeurBonusCourrant_M_DUO$			 <br>		
	...
	/*
		...
		*/
		...
		$SommedesBonusMagasinActuel = $SommedesBonusMagasinActuel +$TotalBonus_AD+$TotalBonus_AI+$TotalBonus_M;
