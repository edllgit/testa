...
$subject      = "Report Redos GKB between $datedebut and $datefin";
...
$rptQuery  = "SELECT user_id, order_num, order_date_processed, redo_order_num, order_product_name, orders.redo_reason_id, cost, order_product_id
...
AND redo_order_num IS NOT NULL AND prescript_lab = 69
AND order_date_processed BETWEEN '$datedebut' AND '$datefin'
AND redo_reasons.redo_reason_id <> '' AND redo_reasons.redo_reason_id <> 0
AND redo_reasons.redo_reason_id NOT IN (37,38,39,46,51,52,57,40,62,43,59,45,65,66,64,31,53,63,42,50)
ORDER BY order_num";	
...
<thead>
    <tr>
        ...
        <th align=\"center\"><b>Cost (CAD)</b></th>
    </tr>
</thead>";
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
    ...
    if ($PrescriptLab <> 69){//Si l'original est faite chez Swiss, on l'ajoute au rapport
...
if ($ordersnum!=0 && !$isExporting){
    ...
    if($response){ 
        //log_email("REPORT: Redos report GKB",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
        echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully to:'. $EmailEnvoyerA.'</strong></div>';
    }else{
        //log_email("REPORT: Redos report GKB",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
...
if(!$isExporting){
    ...
    VALUES('Email redirection Redo report GKB', '$time','$today','$timeplus3heures','cron_send_redirection_redo_report_gkb.php')"; 					
