...
$level 		  = 'Requisitions Acces succursale';
...
$query     = "SELECT * FROM succursales WHERE succ_user_id = '$_POST[inputusername]' AND succ_mot_de_passe = '$_POST[inputpassword]'";
//echo $query;
...
if ($Redirection <> 'no'){
	if ($DataLogin[succ_edll_ou_hbc]=='edll'){
		$_SESSION[idMobile] =$_POST[inputusername];
		$_SESSION[succ_edll_ou_hbc]='edll';
		session_write_close();
		header("Location: gestion_requisitions_succursale_edll.php");/* go to EDLL home page */
	}elseif ($DataLogin[succ_edll_ou_hbc]=='hbc'){
		$_SESSION[idMobile] =$_POST[inputusername];
		$_SESSION[succ_edll_ou_hbc]='hbc';
		session_write_close();
		header("Location: gestion_requisitions_succursale_hbc.php");/* go to HBC  home page */
	}
}	
if ($Redirection == 'no'){
