...
?>
  <head>
    ...
    <title>Ajout d'item dans la réquisition EDLL #<?php echo $ID_Requisition; ?></title>
    <!-- Bootstrap core CSS -->
  ...
  <body>
  <form name="ajout_item_requisition_edll" method="post" action="ajout_item_requisition_edll.php">
   

  <?php  include("inc/menu.inc.php"); ?>
	  
<div class="container">
<b>Ajout d'item dans la réquisition EDLL #<?php echo $ID_Requisition; ?></b>   <a href="modifier_requisition_edll.php?req_id=<?php echo $ID_Requisition; ?>">Retour au détail de la réquisition #<?php echo $ID_Requisition; ?></a>
...
$queryFournisseur="SELECT fourn_nom_fr FROM fournisseurs WHERE fourn_id= $listItem[fournisseur] AND fourn_edll_ou_hbc='edll' AND fourn_status=1";
...
<?php
  		$QueryFournisseur="SELECT * FROM fournisseurs WHERE fourn_edll_ou_hbc='edll' AND fourn_edll_ou_hbc='edll' AND fourn_status=1 ORDER by fourn_nom_fr"; /* select all openings */
...
<?php
  		...
  		$queryCategories="SELECT * FROM categories WHERE cat_edll_ou_hbc='edll' and cat_status=1 ORDER by cat_nom_fr"; /* select all openings */
...
$queryProduitsCorrespondants	= "SELECT * FROM produits WHERE prod_edll_ou_hbc='edll' $FiltreCategorie $FiltreFournisseur AND prod_status=1 ORDER BY produits.categorie,produits.prod_description_fr";
...
<select name=\"prod_req_quantite[]\" class=\"formField2\" id=\"prod_req_quantite[]\">                    
 ...
 echo "<input name=\"prod_id[]\" type=\"hidden\" id=\"prod_id[]\" value=\"$DataProduitsCorrespondants[prod_id]\"/>";
	...
	$DataFourn=mysqli_fetch_array($resultFourn,MYSQLI_ASSOC);
		...
		echo '<td align="center">'.$DataProduitsCorrespondants[prod_nombre_item_inclus].'</td>';
