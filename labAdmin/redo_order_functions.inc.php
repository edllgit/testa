<?php 
//June 2022
function updateRedoOrderV3($pkey){//PRESCRIPTION ORDER UPDATE



//echo 'Contenu de la variable session:<br><br>'. var_dump($_SESSION).'<br><br>';

include "../sec_connectEDLL.inc.php";
	//echo 'V3 in progress...<br>';
	$eye=$_POST['eye'];
	$order_product_id=$_POST['product_id'];
	$user_id=$_POST['user_id'];
	
	//echo '<br>Eye:'. $eye.'<br>';	
	//echo '<br>Product ID:'. $order_product_id.'<br>';	
	//echo '<br>user_id:'. $user_id.'<br><br>';	
	
	
$userQuery="SELECT * FROM accounts WHERE user_id='$user_id'";//GET ORDER INFO
	
//echo ' $userQuery:'. $userQuery . '<br><br>';
...
switch($userItem[product_line]){
	case 'safety':	 	$prodQuery="SELECT * from safety_exclusive WHERE primary_key='$order_product_id'"; break;
	case 'lensnetclub':	$prodQuery="SELECT * from exclusive WHERE primary_key='$order_product_id'";	       break;
	case 'directlens':	$prodQuery="SELECT * from exclusive WHERE primary_key='$order_product_id'";	  	   break;
	case 'aitlensclub':	$prodQuery="SELECT * from exclusive WHERE primary_key='$order_product_id'";  	   break;
	case 'ifcclubca':	$prodQuery="SELECT * from ifc_ca_exclusive WHERE primary_key='$order_product_id'"; break;
	default :		    $prodQuery="SELECT * from exclusive WHERE primary_key='$order_product_id'";	       break;
}
	
//echo '$prodQuery: '. $prodQuery . '<br><br>';
...
//echo 'Currency:' . $userItem['currency'].'<br><br>';	
	
if ($userItem[product_line] <> 'safety')
{
	if ($userItem['currency']=="US"){
		$order_product_price=$prodItem[price];}
	else if ($userItem['currency']=="CA"){
		$order_product_price=$prodItem[price_can];}
	else if ($userItem['currency']=="EUR"){
		$order_product_price=$prodItem[price_eur];}	
	if ($userItem['e_lab']=="E-Lab US"){
		$order_product_price=$prodItem[e_lab_us_price];}	
	else if ($userItem['e_lab']=="E-Lab CAN"){
		$order_product_price=$prodItem[e_lab_can_price];}
}else{
	//Customer is using SAFETY, we need to know which plan he is on	(regular, interco or discounted prices)
	switch($userItem[safety_plan])
	{
		case 'regular price': 	 	$order_product_price=$prodItem[price];			   break;
		case 'interco price': 	 	$order_product_price=$prodItem[price_interco];	   break;
		case 'discounted price': 	$order_product_price=$prodItem[price_discounted];  break;	
	}
}//End IF product line is not SAFETY
...
if (($eye=="R.E.")||($eye=="L.E.")){
	...
	//echo '<br>Order Product Price:'.$order_product_price.'<br><br>';
		

	$order_product_coating = $Lecoating;
	//echo '<br>order_product_coating:'.$order_product_coating.'<br><br>';
	
	
	$order_product_photo=$_POST[order_product_photo];
	//echo '<br>order_product_photo:'.$order_product_photo.'<br><br>';
	
	
	$order_product_polar=$_POST[order_product_polar];
	//echo '<br>order_product_polar:'.$order_product_polar.'<br><br>';
	
	
	$order_product_index=$_POST[order_product_index];
	//echo '<br>order_product_index:'.$order_product_index.'<br><br>';
	...
	$explication_reprise = mysqli_escape_string($con,$_POST[explication_reprise]);
	

$queryEntryFee = "SELECT  entry_fee FROM orders WHERE primary_key=$pkey";
//echo 'queryEntryFee:' . $queryEntryFee . '<br><br>';	
	
$ResultEntryFee=mysqli_query($con,$queryEntryFee)	or die  ('I cannot select items because: ' . mysqli_error($con));
...
if ($_SESSION['NomTraceReprise'] <> ''){
//Signifie qu'une shape a été uploadé, on doit la sauvegarder dans shape_bk_name et myupload
	$resultQueryShape = "UPDATE orders SET myupload = '" . $_SESSION['NomTraceReprise']. "', shape_name_bk = '" . $_SESSION['NomTraceReprise']. "' WHERE  primary_key=$pkey";
	echo '<br>'. $resultQueryShape;
	exit();
	$resultQueryShape = mysqli_query($con,$resultQueryShape)	or die ( "Query failed: " . mysqli_error($con));		
	$_SESSION['NomTraceReprise'] = '';//On vide ce champ
}
...
$DataOrderNum   = mysqli_fetch_array($resultOrderNum);
...
//Vider les variables pour les prismes, lorsqu'il n'y a pas de prismes.
if ($re_pr_io == 'None'){
	$re_pr_io = '';
}
...
$query.="eye='$eye',";
$query.="explication_reprise='$explication_reprise ',";
...
//Aller cherche la raison de reprise qui a déja été sauvegardée:
$queryRaisonReprise="SELECT redo_reason_id FROM orders WHERE primary_key=$pkey";
$resultRaisonReprise = mysqli_query($con,$queryRaisonReprise)	or die ( "Query failed: " . mysqli_error($con));	
$DataRaisonReprise   = mysqli_fetch_array($resultRaisonReprise,MYSQLI_ASSOC);
$RaisondeReprisedelaReprise =  $DataRaisonReprise[redo_reason_id];
	
	
	
if ($redo_reason_id == 100 || $redo_reason_id == 101 || $RaisondeReprisedelaReprise == 100 || $RaisondeReprisedelaReprise == 101){//Garantie a tout casser v3, on doit envoyer un email
...
if ($_POST[redo_password] <> ''){
	...
	if ($NbrResultatRedoAccess > 0 ){
		...
		require_once('../includes/class.ses.php');
			...
			<body>
			<h3>A new redo has been done</h3><br>
			<p>Order number:  <b>$Item[order_num]</b></p>
			<p>This order has been automatically redirected to Trois-Rivieres<br> 
			Please redirect it where it should go once you received the lenses of the original order.</p>
			</body>
			</html>";
			
			$to_address = array('rapports@direct-lens.com');
			$from_address = 'donotreply@entrepotdelalunette.com';
			$subject      = "New Redo: $Item[order_num] ";
			
			/*$response     = office365_mail($to_address, $from_address, $subject, null, $message);	
			//SEND EMAIL
			if($response){ 
				echo '<div class="alert alert-success" role="alert"><strong>Email Sent Sucessfully</strong></div>';
			}else{
				//echo '<div class="alert alert-danger" role="alert"><strong>Error while sending the email</strong></div>';
			}*/
		...
		if ($job_type == 'Uncut'){//Job non taillé, on doit effacer d'extra_product_orders les elements Frame et Edging
			//$QueryDeleteEPO = "DELETE FROM extra_product_orders WHERE  order_id = $pkey AND category IN ('Edging')";
			$QueryDeleteEPO = "UPDATE extra_product_orders SET job_type='Uncut' WHERE  order_id = $pkey AND category IN ('Edging', 'Frame')";
			//echo 'Query:'. $QueryDeleteEPO;
		...
		if (($job_type == 'remote edging') && ($Edging_orders_count > 0)){//Job taillé-monté, on doit effacer d'extra_product_orders l'élément Edging
			$QueryUpdateJobtype = "UPDATE extra_product_orders  SET job_type = '$job_type' WHERE  order_id = $pkey";
			$resultDeleteEPO=mysqli_query($con,$QueryUpdateJobtype)		or die  ('I cannot delete items because 5: ' . mysqli_error());
		}
		...
		updateExtraProductsRedoV2($_POST,$userItem,$pkey);
					
		$gTotal=calculateTotalV2($Item[order_num]);
		...
		//Mise à jour du type de monture, doit impacter tous les éléments dans 'extra_product_orders' liés à la commande en cours
		if (($pkey<>'') && ($frame_type<>'')){
			$queryFrameType ="UPDATE  extra_product_orders  set frame_type='$frame_type' WHERE  order_id='$pkey' ";
			$resultFrameType=mysqli_query($con,$queryFrameType)	or die  ('I cannot delete items because: ' . mysqli_error($con));
		}
		...
		//PRISM SECTION;
		$query="SELECT * from extra_product_orders WHERE order_id='$pkey' AND category='Prism'";//GET ORDER INFO
		//echo '<br><br>QueryPrism: '.$query;//A SUPPRIMER
		
		$prism="prism";	//Valeur par défaut
		$result=mysqli_query($con,$query)		or die  ('I cannot select items because: ' . mysqli_error($con));
		$ep_orders_count=mysqli_num_rows($result);
		if ($ep_orders_count!= 0){		
			$prismItem=mysqli_fetch_array($result,MYSQLI_ASSOC);
		}//END IF ORDER COUNT
		
		if ($_POST['RE_PR_IO']=="None" || $_POST['RE_PR_IO']=="" ){
			$_POST['RE_PR_IO'] = '';
		}
		
		if ($_POST['RE_PR_UD']=="None" || $_POST['RE_PR_UD']==""){
			$_POST['RE_PR_UD'] = '';
		}
		
		if ($_POST['LE_PR_IO']=="None" || $_POST['LE_PR_IO']=="None"){
			$_POST['LE_PR_IO'] = '';
		}
		
		if ($_POST['LE_PR_UD']=="None" || $_POST['LE_PR_UD']==""){
			$_POST['LE_PR_UD'] = '';
		}
		

		if (($_POST['RE_PR_IO'] == "") &&  ($_POST['RE_PR_UD'] == "") && ($_POST['LE_PR_IO'] == "") && ($_POST['LE_PR_UD'] == "")){
			$prism="no prism";	
		}
		
		//echo '<br>État de prism avant sauvegarde:'. $prism . '<br> Compteur:'. $ep_orders_count.'<br><br>';

		if (($prism=="no prism")&&($ep_orders_count!= 0)){
			if ($prismItem[ep_order_id]<>''){
				$query="DELETE FROM extra_product_orders WHERE ep_order_id='$prismItem[ep_order_id]' ";
				//echo '<br><br>DeleteQuery: '.$query;//A SUPPRIMER
				$result=mysqli_query($con,$query)		or die  ('I cannot delete items because: ' . mysqli_error($con));
			}
				
		}else if (($prism=="prism")&&($ep_orders_count==0)){
			//addPrismItem($data,$order_id,$userItem);	
			//echo '<br>Lance la fonction addPrismItem';
		}
		...
		echo '<div class="alert alert-success" role="alert"><strong>Redo V3 Sucessfully Saved/Reprise sauvegardé avec succès</strong></div>';
...
//June 2019
function updateRedoOrderV2($pkey){//PRESCRIPTION ORDER UPDATE

include "../sec_connectEDLL.inc.php";
	//echo 'V2 in progress...<br>';
	$eye=$_POST['eye'];
	$order_product_id=$_POST['product_id'];
	$user_id=$_POST['user_id'];
...
//Signifie qu'une shape a été uploadé, on doit la sauvegarder dans shape_bk_name et myupload
	...
	//echo "Contenu champ session". $_SESSION['RedoData']['myupload'];		
}
...
if ($RaisondeReprisedelaReprise == 65){//Garantie a tout casser, on doit envoyer un email
	$internal_note = $internal_note . ' Ancien compte:' .  $USERID;
	$patient_ref_num =  'Ancien compte:' .  $USERID;;
	$query.=",patient_ref_num='$patient_ref_num'";
	$query.=",user_id='garantieatoutcasser',internal_note='$internal_note'";
...
if ($_POST[redo_password] <> ''){
	...
	$queryRedoAccess  	    = "SELECT * FROM access_redo WHERE password='$RedoPassword'";
	//echo $queryRedoAccess;
	$resultRedoAccess 	    = mysqli_query($con,$queryRedoAccess)	or die ( "Query failed: " . mysqli_error());	
	$NbrResultatRedoAccess  = mysqli_num_rows($resultRedoAccess);
}
	...
	if ($NbrResultatRedoAccess > 0 ){
		...
		$queryHistory = "INSERT INTO status_history (order_num,     update_time,     update_type, order_status,   update_ip,    update_ip2,     redo_approved_by) 
											 VALUES ($OrderNumber, '$datecomplete',   'manual',     'processing',               '$ip',       '$update_ip2',  '$DataRedoAccess[name]')";
		$result=mysqli_query($con,$queryHistory)	or die ( "Query failed: " . $queryHistory  . mysqli_error($con));	
		...
		//$queryUpdateFrame = "UPDATE extra_product_orders SET price  = 0 WHERE order_id = $pkey AND  category= 'Frame'";
		//echo '<br>QueryUpdateFrame:'. $queryUpdateFrame;
		...
		if ($job_type == 'Edge and Mount'){
			//Dans le cas ou la job était 'Remote edging' on efface les données de la shape qui aurait été attachée
			//$QueryRemoveShape  = "UPDATE orders  SET myupload = '', 	shape_name_bk = ''  WHERE  primary_key = $pkey";
			//$resultRemoveShape = mysqli_query($con,$QueryRemoveShape)		or die  ('I cannot delete items because 3: ' . mysqli_error($con));
		}
			
		//Enlever les dispensing fee
		if (($userItem[product_line] == 'ifcclubca')||($userItem[product_line] == 'safety')){
			$QueryDeleteDispFee = "DELETE FROM extra_product_orders WHERE  order_id = $pkey AND category like '%dispensing%'";
			$resultDeleteEPO=mysqli_query($con,$QueryDeleteDispFee)		or die  ('I cannot delete items because: ' . mysqli_error($con));
		}
		...
		$Query="SELECT * FROM orders WHERE primary_key='$pkey'";//GET ORDER INFO
		$Result=mysqli_query($con,$Query)	or die  ('I cannot select items because: ' . mysqli_error($con));
		$Item=mysqli_fetch_array($con,$Result);
		...
		echo '<div class="alert alert-success" role="alert"><strong>Redo V3 Sucessfully Saved</strong></div>';
...
function updateExtraProductsRedoV2($data,$userItem,$pkey){
...
//FRAME SECTION iciici;
$query="SELECT * from extra_product_orders WHERE order_id='$order_id' AND category='Frame'";//GET ORDER INFO
$result=mysqli_query($con,$query) or die  ('I cannot select items because: ' . mysqli_error($con));
$ep_orders_count=mysqli_num_rows($result);
if ($ep_orders_count!= 0){
...
if (($data['order_type']!="Provide")&&($ep_orders_count!= 0)){
	$query="DELETE FROM extra_product_orders WHERE ep_order_id='$frameItem[ep_order_id]' ";
	//echo $query;
	//$result=mysqli_query($con,$query) or die  ('I cannot delete items because: ' . mysqli_error($con));
}else if (($data['order_type']=="Provide")&&($ep_orders_count!=0)){
	updateFrameItem($data,$order_id,$userItem,$frameItem[ep_order_id]);
}else if (($data['order_type']=="Provide")&&($ep_orders_count==0)){
	addFrameItem($data,$order_id,$userItem);
}
...
function addTintItem($data,$order_id){//ADD TINT PRODUCT ITEM TO ORDERS DB
	...
	//Aller chercher l'index utilisé dans la commande 
	$queryIndex  = "SELECT order_product_index  FROM orders WHERE primary_key= $order_id";
	//echo '<br>'.$queryIndex.'<br>';
	$ResultIndex =  mysqli_query($con,$queryIndex) or die ( "Insert Engraving query failed: " . mysqli_error($con));
	$DataIndex 	 =  mysqli_fetch_array($ResultIndex,MYSQLI_ASSOC);
	$Index 		 = $DataIndex[order_product_index];
	
	if ($Index==1.5){
		//Indice = Puisque l'indice est de 1.50, on doit ajouter un extra UV400	
		$queryUV400 = "UPDATE ORDERS SET UV400='UV400' WHERE primary_key= $order_id";	
		//echo '<br>'.$queryUV400. '<br>';
		$resultUV400 = mysqli_query($con,$queryUV400) or die ( "UV400 insertion failed: " . mysqli_error($con));
	}//END IF
	...
	//UPDATE NEW ORDER_TOTAL
	$queryNewOrderTotal = "UPDATE orders SET order_total = $NEW_OrderTotal  WHERE primary_key = $order_id ";
	//echo "<br>QueryNewOrderTotal1:$queryNewOrderTotal<br>";
	$resultNewOrderTotal =mysqli_query($con,$queryNewOrderTotal)		or die ( "Insert Tint query failed: " . mysqli_error($con) );
}
...
function addFrameItem($data,$order_id,$userItem){//ADD FRAME PRODUCT ITEM TO ORDERS DB
	...
	echo 'Fonction AddFrameItem<br>Query'.$query ;
...
function updateFrameItem($data,$order_id,$userItem,$ep_order_id){//ADD FRAME PRODUCT ITEM TO ORDERS DB
	include "../sec_connectEDLL.inc.php";
	echo 'fonction updateFrameItem<br>'; 
