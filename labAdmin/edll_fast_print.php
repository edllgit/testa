...
switch($_SESSION["accessid"]){
		case 191: $User_ID_IN = " ('laval','lavalsafe','garantieatoutcasser','redosafety') ";           		break;        
		case 192: $User_ID_IN = " ('entrepotifc','entrepotsafe','garantieatoutcasser','redosafety') "; 	 		break;      
		case 193: $User_ID_IN = " ('entrepotdr','safedr','garantieatoutcasser','redosafety','GARAGEMP') "; 		break;
		case 199: $User_ID_IN = " ('terrebonne','terrebonnesafe','garantieatoutcasser','redosafety') "; 		break;        
		case 203: $User_ID_IN = " ('sherbrooke','sherbrookesafe','garantieatoutcasser','redosafety') "; 		break;       
		case 205: $User_ID_IN = " ('chicoutimi','chicoutimisafe','garantieatoutcasser','redosafety') "; 		break;   
		case 207: $User_ID_IN = " ('levis','levissafe','garantieatoutcasser','redosafety') ";	        		break;
		case 208: $User_ID_IN = " ('longueuil','longueuilsafe','garantieatoutcasser','redosafety') ";  	 		break;
		case 210: $User_ID_IN = " ('granby','granbysafe','garantieatoutcasser','redosafety') ";	        		break;
		case 221: $User_ID_IN = " ('entrepotquebec','quebecsafe','garantieatoutcasser','redosafety','redoqc') ";break;	
		case 196: $User_ID_IN = " ('warehousehal','warehousehalsafe','garantieatoutcasser','redosafety') ";     break;
		//case 228: $User_ID_IN = " ('montreal','montrealsafe','garantieatoutcasser','redosafety') ";     		break;
		case 230: $User_ID_IN = " ('stjerome','stjeromesafe','garantieatoutcasser','redosafety') ";    		 	break;
		case 231: $User_ID_IN = " ('gatineau','garantieatoutcasser','redosafety') ";    						break;
		case 240: $User_ID_IN = " ('edmundston','edmundstonsafe','garantieatoutcasser','redosafety') ";    		break;
		case 242: $User_ID_IN = " ('vaudreuil','vaudreuilsafe','garantieatoutcasser','redosafety') ";    		break;
...
if($_POST[rpt_search]=="Verifier")
{
	...
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			$errorMessage .= "<br>Invalid Order number (should be 7 caracters)";
			...
			WHERE orders.order_num IN ($_POST[order_num]) AND LAB  IN(59,66,67)
 ...
 if($_POST[rpt_search_batch]=="Verifier")
{
	...
	if (strlen($_POST["dernier_order_num"])<> 7)
	{
		//$errorMessage .= "<br><strong>$valueSansEspace</strong>: Invalid Order number (should be 7 caracters)";
		$errorMessage .= "<br>Invalid Order number (should be 7 caracters)";
		$PassValidation = false;
	...
	if (strlen($_POST["premier_order_num"])<> 7)
	{
		//$errorMessage .= "<br><strong>$valueSansEspace</strong>: Invalid Order number (should be 7 caracters)";
		$errorMessage .= "Invalid Order number (should be 7 caracters)";
		$PassValidation = false;
	}
...
//echo '<br>Nbr commande dans le range:'. 	$NombredeCommande;
			...
			LEFT JOIN (buying_groups) ON (accounts.buying_group = buying_groups.primary_key) 
			LEFT JOIN (labs) ON (orders.lab = labs.primary_key) 
			WHERE orders.order_num BETWEEN $_POST[premier_order_num] AND $_POST[dernier_order_num] AND LAB  IN(3) and orders.user_id='firststreet1476dl' GROUP BY order_num ORDER BY order_num";
			//	echo $rptQuery;
			}else{
...
foreach( $_POST[UpdateOrderNum] as $the_order_num ){	
	...
	if (strlen($the_order_num)==7)
	{
		...
		$resultJobType = mysqli_query($con,$queryJobType)		or die ('Could not insert because: ' . mysqli_error($con));
		...
		//We update the 'frame_sent_to_stc' field
		...
		//echo '<br>' . $queryUpdate;
		//$UpdateDetails  .= "<br>Order #$the_order_num tray has been updated to: $TrayValue";
...
<html>
<head>
<title>Edll Fast Printing Tool</title>
...
<form  method="post" name="verification" id="verification" action="edll_fast_print.php">
            <table width="100%" border="0" cellpadding="2" cellspacing="0" class="formField">
            	<tr bgcolor="#000000">
            		<td align="center" colspan="4"><b><font color="#FFFFFF" size="1" face="Helvetica, sans-serif, Arial">EDLL Fast Printing Tool</font></b></td>
            	</tr>
				...
				<tr bgcolor="#DDDDDD">
					<td align="center" valign="middle" nowrap bgcolor="#DDDDDD" ><div style="font-size:14px;" align="center">
                        ...
                        <br><br><br><br><div><strong>1-</strong> Svp entrer vos numéros de commande séparé par une virgule</div>					<textarea cols="20" name="order_num" style="font-size:16px;"  rows="10" id="order_num" class="formField"><?php if($_POST[rpt_search]=="Verifier"){
