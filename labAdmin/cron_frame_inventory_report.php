<?php
//Rapport d'inventaire de Conant (IFC CLUB)
include('../includes/phpmailer_email_functions.inc.php');
require_once('../includes/class.ses.php');
include("../Connections/sec_connect.inc.php");
$lab_id = 37; //IFC FRANCE
$today=date("Y-m-d");
$rptQuery="SELECT ifc_frames_french.*, product_inventory_ifc.product_inventory_id, product_inventory_ifc.min_inventory, product_inventory_ifc.inventory, product_inventory_ifc.last_updated, product_inventory_ifc.product_id
		...
		WHERE collection NOT IN ('PREM','PREM PLUS','RENDEZVOUS','BUGETTI','RX','ISEE','MASSIMO','MILANO 6769','GO IWEAR','DI GIANNI','ARMOURRX','MODELLI','HAGGAR','MONTURE','POLAR','Wrap-Rx','Metro','Classic','Basic' )
		ORDER BY code";
echo $rptQuery . '<br>';
...
$subject="Conant Rapport d'inventaire des montures";	
...
//Log email
	...
	if($response){ 
		log_email("REPORT: Frame inventory IFC France",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
    }else{
		log_email("REPORT: Frame inventory IFC France",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
	}	
