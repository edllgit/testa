...
require_once("../barcodes/Barcode39.php");
// set object
...
$bc->draw("$orderNum".".gif");
	...
	$message='<html xmlns="https://www.w3.org/1999/xhtml"><head>
				...
				while ($e_listItem=mysqli_fetch_array($e_result,MYSQLI_ASSOC)){
						...
						if ($e_listItem[category]=="Edging"){
							...
							if ($e_listItem[category]=="Iimpact"){
								...
								$e_order_string_tint.="<b>Iimpact: </b>";
							}
							...
							if ($e_listItem[category]=="Prism"){
								if ($Product_line=='eye-recommend'){
									...
									$resultPrisme = mysqli_query($con,$queryPrisme) or die 
									('I cannot select Prism items because: ' . mysqli_error($con).$queryPrisme);
...
$message .= '</head>';
	...
	$query="SELECT po_num, order_date_processed, order_date_shipped, redo_order_num FROM orders WHERE order_num='$orderNum'"; //GET ORDER PO NUMBER
	$result=mysqli_query($con,$query)		or die  ('I cannot select items because: ' . mysqli_error($con));
	$listItem=mysqli_fetch_array($result,MYSQLI_ASSOC);
	//iciici
	$message.='<table width="650" border="0" align="center" cellpadding="3" cellspacing="0"  class="formBox"><tr><td align="left" class="formCellNosides">PO Order #: ';
...
switch($order_product_type){
	case 'exclusive':        sendPrescriptionConfirmation($fromAddress,$logo_file,$orderNum,$send_to_address,$user_id,$userData,$Displayprice,false);  	 break;
