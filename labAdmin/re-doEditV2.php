...
include "../sec_connectEDLL.inc.php";
	...
	$queryPO    = "SELECT po_num, order_date_processed, order_date_shipped, redo_order_num, order_num_optipro, order_num_opticbox, prescript_lab, patient_ref_num, eye FROM  orders WHERE order_num=$orderNum"; //GET ORDER & REDO ORDER #
	$resultPO   = mysqli_query($con,$queryPO) or die  ('I cannot select items because 4a: <br>QUERY PO :' . $queryPO .'<br>' . mysqli_error($con));
	$listItem = mysqli_fetch_array($resultPO,MYSQLI_ASSOC);
...
//echo 'get PK: ' .$_GET[pkey] . '<br>';
//echo 'POST PK: '.$_POST[pkey]. '<br>';
...
//echo 'Session  :'.$_SESSION['RedoData']['myupload'];
}					
...
if (($_POST[confirm_redo_reason_selection]=="Confirm Selected Redo Reason") || ($_POST[confirm_redo_reason_selection]=="Confirmer la raison de reprise sélectionnée")){
	...
	$pkey			= $_POST['pkey'];
	//echo '<br>Passe dans save redo reason: pkey:'.$pkey	;
	...
	if (($pkey<>'') && (strlen($pkey)==6)){
		...
		//Si la raison de reprise est 'Mauvaise Courbure de base Reçue': il faut aller valider si une courbure de base avait été spécifié dans la commande originale 
		// Cette raison de reprise correspond à la raison #23
		
		if ($redo_reason_id == 23){
			//Aller chercher la valeur du champ 'Base_Curve' pour la commande originale
			$queryOriginalOrder	 = "SELECT base_curve FROM orders WHERE order_num = (SELECT redo_order_num FROM orders WHERE primary_key =$pkey)";
			//echo $queryOriginalOrder;
			$resultOriginalOrder = mysqli_query($con,$queryOriginalOrder)	or die  ('I cannot select items because 1:' . $queryOriginalOrder.'<br>' . mysqli_error($con));
			$DataOriginalOrder 	 = mysqli_fetch_array($resultOriginalOrder,MYSQLI_ASSOC);
			//echo '<br>Base Curve  Commande Originale: '.$DataOriginalOrder[base_curve];
			$LongeurBaseCurve = strlen($DataOriginalOrder[base_curve]);
			//exit();
			if ($LongeurBaseCurve>0){//Signifie qu'une courbure de base a été demandé
				//Aucun traitement pour le moment lorsque cette condition est respectée.
 			}else{
				//Aucune courbure n'a été spécifée dans la commande originale
				//Il faut donc: 
				//1-Stopper cette reprise  et afficher un message clair et approprié
				echo '<br><br>
				<p align="center"><u><b>MESSAGE IMPORTANT A PROPOS DE VOTRE REPRISE</b></u>:</p> 
				<p align="center">Aucune courbure de base n\'a été spécifié dans votre commande originale, vous ne pouvez donc pas faire de reprise avec la raison <b>\'Mauvaise courbure reçue</b>.\' 
				<br>Si vous avez des questions, svp contacter <a href="mailto:dbeaulieu@direct-lens.com">Danielle Bouffard</a>.
				<br><br>
				<a href="re-doEditV2.php?pkey='.$pkey.'">Retour vers votre reprise (Vous ne pourrez pas terminer cette reprise)</a></p>
				
				<br><br>
				
				<p align="center"><u><b>IMPORTANT MESSAGE ABOUT YOUR REDO</b></u>:</p>
				<p align="center">No base curve have been specified in the original order, therefore, you cannot create a redo with the reason \'<b>Wrong base curve received</b>.\'
				<br>If you have questions about this, please contact <a href="mailto:dbeaulieu@direct-lens.com">Danielle Bouffard</a>.
				<br><br>
				<a href="re-doEditV2.php?pkey='.$pkey.'">Back to Order (You won\'t be able to finish this redo)</a></p>';
				
				//2-Aviser Danielle Bouffard par Courriel de la situation
				
				
				//3-Mettre la reprise au status 'cancelled'
				$queryUpdateStatus = "UPDATE orders set order_status = 'cancelled' WHERE primary_key = $pkey";
				//echo '<br><br><br>queryUpdateStatus:'. $queryUpdateStatus;
				$resultUpdateStatus=mysqli_query($con,$queryUpdateStatus)	or die  ('I cannot select items because 1: ' . mysqli_error($con));
				
				//SAUVEGARDER UNE RAISON DE REDO OBSCURE AFIN QU'ILS NE PUISSENT PAS POURSUIVRE LE TRAITEMENT
				$querySaveRedoReason = "UPDATE orders set redo_reason_id = 73 WHERE primary_key = $pkey";
				...
				//header("Location:re-doEditV2.php?pkey=$pkey");
				exit();
			}//End IF
...
<script language="JavaScript" type="text/javascript">
function VerifyIfAReasonIsSelected()
{
	$SelectedRedoReason = document.getElementById("redo_reason_id").value;
	//alert($SelectedRedoReason);
	//A)Lorsqu'aucune raison n'est sélectionnée: $SelectedRedoReason = '' AKA (vide) <br>
	//B)Lorsqu'une raison est sélectionnée, on recoit son ID 
	//B)EXEMPLE: COATING | Sun Coating Defect (20%) = ID #54
	switch($SelectedRedoReason){
		case '':  	$RaisonValide = 'invalide'; break;//Invalide, aucune raison de reprise n'est sélectionnée		
		default:   	$RaisonValide = 'valide';	break;//Valide, une raison de reprise est sélectionnée			
			
	}
	//Si une raison de reprise valide est sélectionnée, on active le bouton de confirmation C) Sinon, on le désactive
	if($RaisonValide=='valide'){//= Une raison de reprise est sélectionnée		
	document.getElementById("confirm_redo_reason_selection").disabled = false;
	//document.getElementById("SubmitEyeChange").disabled = true;		
	}else{
	document.getElementById("confirm_redo_reason_selection").disabled = true;
	//document.getElementById("SubmitEyeChange").disabled = false;	
...
{
	//DÉSACTIVATIONS
	//Désactiver le menu des raisons de reprises
	document.getElementById("redo_reason_id").disabled = true;	
	//Désactiver le bouton Confirmation raison de reprise
	document.getElementById("confirm_redo_reason_selection").disabled = true;
	//Désactiver le bouton pour changer l'oeil inclus dans la reprise
	document.getElementById("SubmitEyeChange").disabled = true;
	
	//ACTIVATION
	//Activer champ suivante: Origine de la reprise
	document.getElementById("redo_origin").disabled = false;
}
</script>	
	
	
<script language="JavaScript" type="text/javascript">
function VerifyOrigineRedo()
{
	
	$SelectedRedoOrigin = document.getElementById("redo_origin").value;
	switch($SelectedRedoOrigin){
		case '':  	$OrigineValide = 'invalide'; break;//Invalide, aucune origine  n'est pas connue		
		default:   	$OrigineValide = 'valide';	 break; //Valide,l'origine est connnue			
			
	}
	
	//Si l'origine est renseignée, on continue sinon, on ne fait rien.
	if($OrigineValide=='valide'){
		document.getElementById("confirm_redo_reason_selection").disabled = true;
		document.getElementById("SubmitEyeChange").disabled = true;	
		document.getElementById("redo_password").disabled = false;
	}else{
		document.getElementById("confirm_redo_reason_selection").disabled = false;
		document.getElementById("SubmitEyeChange").disabled = false;
		document.getElementById("redo_password").disabled = true;
	}
	
	//Fonctionne jusqu'ici	
	//ACTIVER TOUS LES CHAMPS QUI DEVRAIENT l'ËTRE SELON LA RAISON DE REPRISE SÉLECTIONNÉE
...
function ActiverChampEtSoumettreForm(){
	...
	//1-Ré-activer les champs avant de soumetre le formulaire
	//PATIENT
	document.getElementById("patient_ref_num").disabled=false; 
	document.getElementById("order_patient_first").disabled=false; 	
	document.getElementById("order_patient_last").disabled=false; 
	document.getElementById("tray_num").disabled=false; 
	//ADDITIONAL ITEM
	document.getElementById("extra_product").disabled=false; 
	document.getElementById("extra_product_price").disabled=false; 
	//LA RX	
	//Sphères
	document.getElementById("re_sphere").disabled=false;
	document.getElementById("le_sphere").disabled=false;
	//Cylindres
	document.getElementById("re_cyl").disabled=false;
	document.getElementById("le_cyl").disabled=false;	
	//Additions
	document.getElementById("le_add").disabled=false;	
	document.getElementById("re_add").disabled=false;
	//PD Loin
	document.getElementById("le_pd").disabled=false;	
	document.getElementById("re_pd").disabled=false;
	//PD près
	document.getElementById("le_pd_near").disabled=false;	
	document.getElementById("re_pd_near").disabled=false;
	//3 mesures Personnalisés
	...
	//Axes	
	document.getElementById("re_axis").disabled=false;
	document.getElementById("le_axis").disabled=false;
	//Épaisseurs Spéciales	
	document.getElementById("RE_CT").disabled=false;
	document.getElementById("LE_CT").disabled=false;	
	document.getElementById("LE_ET").disabled=false;
	document.getElementById("RE_ET").disabled=false;	
	//Les hauteurs d'ajustement
	document.getElementById("le_height").disabled=false;	
	document.getElementById("re_height").disabled=false;
	//AUTRES
	document.getElementById("optical_center").disabled=false;
	document.getElementById("base_curve").disabled=false;	
	document.getElementById("mirror").disabled=false;	
	//AUTRES TOUJOURS ACTIVÉS
	document.getElementById("job_type").disabled=false;
	document.getElementById("special_instructions").disabled=false;	
	document.getElementById("internal_note").disabled=false;
	//DÉTAIL TEINTE
	document.getElementById("tint").disabled=false;		
	document.getElementById("tint_color").disabled=false;			
	document.getElementById("from_perc").disabled=false;
	document.getElementById("to_perc").disabled=false;
		
	/*MONTURE*/	
	//Mesure de la monture
	document.getElementById("frame_a").disabled=false;	
	document.getElementById("frame_b").disabled=false;	
	document.getElementById("frame_dbl").disabled=false;	
	document.getElementById("frame_ed").disabled=false;
	document.getElementById("frame_type").disabled=false;	
	//Détail de la monture	
	document.getElementById("temple_model_num").disabled=false;
	document.getElementById("color").disabled=false;
	document.getElementById("supplier").disabled=false;
	//Trace
	document.getElementById("file").disabled=false;
		
	//Index-Photo-Polar-Produit-Origine
	document.getElementById("order_product_index").disabled=false;
	document.getElementById("order_product_photo").disabled=false;
	document.getElementById("order_product_polar").disabled=false;	
	document.getElementById("product_id").disabled=false;
	document.getElementById("redo_origin").disabled=false;
	
	//GTC/TT
	document.getElementById("redo_detail").disabled=false;
	
	//PRISMES
	//Oeil Gauche
	document.getElementById("LE_PR_IO1").disabled=false;
	document.getElementById("LE_PR_IO2").disabled=false;
	document.getElementById("LE_PR_IO3").disabled=false;
	document.getElementById("LE_PR_UD1").disabled=false;
	document.getElementById("LE_PR_UD2").disabled=false;
	document.getElementById("LE_PR_UD3").disabled=false;
	document.getElementById("le_pr_ax").disabled=false;
	document.getElementById("le_pr_ax2").disabled=false;
	//Oeil Droit
	document.getElementById("RE_PR_IO1").disabled=false;
	document.getElementById("RE_PR_IO2").disabled=false;
	document.getElementById("RE_PR_IO3").disabled=false;
	document.getElementById("RE_PR_UD1").disabled=false;
	document.getElementById("RE_PR_UD2").disabled=false;
	document.getElementById("RE_PR_UD3").disabled=false;
	document.getElementById("re_pr_ax").disabled=false;
	document.getElementById("re_pr_ax2").disabled=false;
...
<script>
function ActivateFieldsByRedoReason() {
$redoReason = document.getElementById("redo_reason_id").value;	

switch($redoReason){
	//GROUPE A: Tous les changements sont permis
	case '75': 	//PRODUCT | Swiss product no longer available
	case '31': 	//ERROR | Data Entry Error 
	case '52': 	//NON ADAPT | Change Index
	case '53': 	//NON ADAPT | Change Frame
	case '57': 	//NON ADAPT | Change type of product       
	case '69': 	//ERROR | Staff Manipulation 
	case '70': 	//ERROR | Data Entry Error 
	//LAB ONLY: 
	case '1' : 	//LAB USE ONLY | Coating Defect (LAB USE ONLY)	
	case '42': 	
	document.getElementById("patient_ref_num").disabled=false; 	//LAB USE ONLY | Lab Edging Error (LAB USE ONLY)
	document.getElementById("redo_password").disabled = false;	
	...
	//GROUPE B: Aucun changement n'est permis:[FONCTIONNEL EN CE MOMENT: 20 Juin 2019]
	case '2' : //COATING | Scratched
	case '3' : //COATING | Pealed
	case '4' : 	//COATING | Wrong Coating
	case '10': 	//RX | Wrong RX	
	case '14': 	//RX | Wrong Prism
	case '15': 	//RX | Received Wrong Pd
	case '16': 	//NON ADAPT | Distortion	
	case '17': 	//PRODUCT | Wrong Product
	case '18': 	//PRODUCT | Too Small	
	case '22': 	//PRODUCT | Wrong Tint
	case '23': 	//PRODUCT | Wrong Base curve
	case '36': 	//SHIPPING | Received Broken
	case '44': 	//EDGING | Bad Remote Edging 
	case '48': 	//EDGING | Receive wrong Fitting Height
	case '54': 	//COATING | Sun Coating Defect
	case '56': 	//PRODUCT | Micro-Engraving	
	case '68': 	//PRODUCT | Requested Thickness not respected
	case '71': 	//EDGING | Aesthetic Defect
	case '72': 	//EDGING | Spin
	...
	//Ajouté 19 Avril 2021 pour temporairement permettre de changer le produit pour choisir le bon corridor.
	//Index-Photo-Polar-Produit	
	document.getElementById("product_id").disabled=false;	
	...
	//GROUPE C: CERTAINS changents sont permis                           CHANGEMENTS PERMIS
		
	case '39': //NON ADAPT | Incorrect Fitting Height             		 Les hauteurs + le produit
	//Les hauteurs d'ajustement
	document.getElementById("le_height").disabled=false;	
	document.getElementById("re_height").disabled=false;
	//AUTRES TOUJOURS ACTIVÉS
	...
	//LE PRODUIT
	//Index-Photo-Polar-Produit	:[Ajouté 27 SEPT 2019 D Bouffard]
	document.getElementById("order_product_index").disabled=false;
	document.getElementById("order_product_photo").disabled=false;
	document.getElementById("order_product_polar").disabled=false;	
	document.getElementById("product_id").disabled=false;
	...
	case '40': 	//NON ADAPT | Dr RX Change                         		 Toute la RX + le produit
	//LA RX	
	//Sphères
	document.getElementById("re_sphere").disabled=false;
	document.getElementById("le_sphere").disabled=false;
	...
	document.getElementById("re_pr_ax").disabled=false;
	document.getElementById("re_pr_ax2").disabled=false;
	//Index-Photo-Polar-Produit	:[Ajouté 5 aout 2019]
	document.getElementById("order_product_index").disabled=false;
	document.getElementById("order_product_photo").disabled=false;
	document.getElementById("order_product_polar").disabled=false;	
	document.getElementById("product_id").disabled=false;		
	...
	case '63'://NON ADAPT | Change PD                            		 Les pds (de près et de loin)
	//PD Loin
	document.getElementById("le_pd").disabled=false;	
	document.getElementById("re_pd").disabled=false;
	//PD près
	...
	case '64': //PRODUCT | Frame not available anymore 		     		 Le choix de la monture, la trace, les hauteurs et le produit(Produit, Index, Photo, Polar
	/*MONTURE*/	
	//Mesure de la monture
	document.getElementById("frame_a").disabled=false;	
	document.getElementById("frame_b").disabled=false;	
	...
	//GROUPE D: Cas d'exceptions                       				CHANGEMENTS PERMIS
	case '46': //NON ADAPT | Change Corridor           				Corridor, Produit 	
	//Index-Photo-Polar-Produit	
	document.getElementById("order_product_index").disabled=false;
	document.getElementById("order_product_photo").disabled=false;
	document.getElementById("order_product_polar").disabled=false;	
	...
	case '65': //GTC ou TTT (Temple to Temple Warranty)	
	document.getElementById("redo_detail").disabled=false;	
	/*MONTURE*/	
	//Mesure de la monture
	document.getElementById("frame_a").disabled=false;	
...
<input style="display:none" type="password" name="foilautofill"/>
  <table border="0" cellspacing="0" cellpadding="0" width="100%">
  	<tr valign="top">
		...
		?></td>
  		...
  		<td width="1" bgcolor="#FFFFFF">&nbsp;</td>
  		<td width="80%">
            <table width="100%" border="1" cellpadding="2" cellspacing="0" class="formField3">
            	<tr bgcolor="#000000">
            		<td colspan="2" align="center"><b><font color="#FFFFFF" size="1" face="Helvetica, sans-serif, Arial">EDIT
			...
			<tr>
             ...
             <tr>
                <td bgcolor="#F0EDEE" colspan="4" align="left">
					 ...
					 <input name="SubmitEyeChange" type="submit" id="SubmitEyeChange" class="formField3" value="Update"></td>
 ...
 <table width="100%" border="0" align="center" cellpadding="3" cellspacing="0"  class="formField3">      
			...
			<form action="re-doEditV2.php" method="post" enctype="multipart/form-data" name="saveRedoReason" id="saveRedoReason">
				...
				<input type="hidden" name="pkey" id="pkey"  value="<?php echo $pkey;?>"></td>
					...
					<tr>
						...
						<?php  if ($mylang == 'lang_french'){
							echo "<strong>B)SELECTIONNER LA RAISON DE LA REPRISE:&nbsp; </strong>";
						}else {
							echo "<strong>B)SELECT THE REDO REASON:</strong>";
						} ?>
						</td>
					</tr>
					...
					<tr>
						<td  colspan="3"  align="left">  
							...
							<select name="redo_reason_id" <?php echo $disabled; ?>  class="form-control" id="redo_reason_id"  onChange="VerifyIfAReasonIsSelected()">
							<?php
							
								
							if ($mylang == 'lang_french'){//Langue == FRANÇAIS
								$queryRedo="SELECT * FROM redo_reasons  WHERE active=1 AND redo_reason_id<76  ORDER BY redo_reason_fr"; /* select all openings */			
							}else{
								$queryRedo="SELECT * FROM redo_reasons  WHERE active=1 AND redo_reason_id<76  ORDER BY redo_reason_en"; /*1= Coating Defect. 42= Lab edging error */	
							}
	 ...
	 <?php if ($disabled<>'disabled'){ ?> 
					<tr>
						<td colspan="8"  align="left" bgcolor="#F0EDEE"><b>
							<?php  if ($mylang == 'lang_french'){
								echo "<strong> C)CONFIRMER LA RAISON DE REPRISE SÉLECTIONNÉE <u>(PVI: Une fois confirmée, il sera impossible de changer la raison)</u></strong>";
							}else{
								echo "<strong> C)CONFIRM SELECTED REDO REASON: <u>(FYI: Once confirmed, it won't be possible to change this reason)</u></strong>";
							}?>
							</b>
						</td>
					</tr>	
					...
					<tr>
						<td colspan="2">
							<?php   if ($mylang == 'lang_french'){ ?>
								<input type="submit"  name="confirm_redo_reason_selection" disabled  id="confirm_redo_reason_selection"  value="Confirmer la raison de reprise sélectionnée" class="formField3" >
							<?php	}else { ?>
								<input type="submit" name="confirm_redo_reason_selection" disabled  id="confirm_redo_reason_selection"  value="Confirm Selected Redo Reason" class="formField3" >
							<?php   }  ?>	
						</td>
					</tr>
			...
			<tr>
				<td>&nbsp;</td>
				 ...
				 <tr>
                    ...
                    <?php  if ($mylang == 'lang_french'){
					echo "<strong> D)SÉLECTIONNER LA CIRCONSTANCE DE LA REPRISE&nbsp; </strong>";
					}else {
					echo "<strong> D)SELECT THE ORIGIN OF THE REDO:</strong>";
					} ?>
                    </strong></td>
                </tr>
                ...
                <tr>
                <td colspan="2">
                <select name="redo_origin" id="redo_origin"  class="form-control" <?php if ($disabled<>'disabled') echo disabled; ?>  onChange="VerifyOrigineRedo();">
				 ...
				 <tr>
                	<td bgcolor="#F0EDEE" colspan="3"><strong>
                    <?php  if ($mylang == 'lang_french'){
					echo "<strong> E)EXPLICATION DE VOTRE REPRISE&nbsp; </strong>";
					}else {
					echo "<strong> D)EXPLANATION OF YOUR REDO:</strong>";
					} ?>
                    </strong></td>
				</tr>
				...
				<tr>
             	  ...
             	  <tr><td>&nbsp;</td></tr>
					...
					<tr bgcolor="#000000">
						<td colspan="8" align="center"><b><font color="#FFFFFF" size="1" face="Helvetica, sans-serif, Arial">&nbsp;</font></b><?php echo $message;?>
						</td>
       		 		</tr>
				  <tr><td>&nbsp;</td></tr>
				...
				<div id="SelectionIndice-Transitions-Polarise">
					<tr>
						...
						<?php  if ($mylang == 'lang_french'){
						echo "<strong> 1-SÉLECTIONNER L'INDICE&nbsp;</strong>";
						}else {
						echo "<strong> 1-SELECT THE INDEX</strong>";
						} ?>
						</strong></td>
					...
					<td width="11%" bgcolor="#FFFFFF">
						<select class="form-control" name="order_product_index" id="order_product_index" OnChange="document.redoForm.submit();" disabled >
						  <?php
					...
					<select name="order_product_photo" id="order_product_photo"  OnChange="document.redoForm.submit();" class="form-control" disabled >
					...
					echo "$name</option>";}}?>
					 ...
					 <td width="16%" colspan="1">
					 <select name="order_product_polar" id="order_product_polar"  OnChange="document.redoForm.submit();"   class="form-control" disabled>
...
//echo 	'<br> Charles: '. $prodQuery . '<br>';
                ...
                <tr>   
                <td colspan="3"><input  class="form-control"   name="redo_detail" title="Mandatory if the redo reason if Other Reason to explain" alt="Mandatory if the redo reason if Other Reason to explain"  type="text" class="formField3" id="redo_detail" disabled value="<?php echo $listItem[redo_detail];?>" size="50" maxlength="50" max="50"></td>
                </tr>
        ...
        <table  width="60%" border="10" align="right" cellpadding="3" cellspacing="0"  class="formField3">   
			...
			<tr>
              ...
              <tr>
					...
					<input name="patient_ref_num" type="text" class="formField3" id="patient_ref_num" value="<?php echo $listItem[patient_ref_num];?>" size="5" disabled>&nbsp;&nbsp;
                ...
                <tr>
					  ...
					  <td align="center" bgcolor="#DDDDDD">
						  <select name="re_sphere" id="re_sphere" disabled>
						  <?php
						  $min = -15.75;
						  $max = 14.75;
						   if ($listItem[eye]!="L.E."){
					  ...
					  <td align="center" bgcolor="#DDDDDD">
						  <select name="re_cyl"  id="re_cyl" disabled>
					  <?php
					  $min= -8.75;
					  $max= 6.75;
					   if ($listItem[eye]!="L.E."){
					  ...
					  <td width="150" align="center" bgcolor="#DDDDDD"><input name="re_axis" type="text" class="formField3" id="re_axis" value="<?php if ($listItem[re_axis]>0) echo $listItem[re_axis];?>" size="4" maxlength="4" disabled></td>


					 <td align="center" bgcolor="#DDDDDD">
						  <select name="re_add"  id="re_add" disabled>
					  <?php
					...
					<td align="center" bgcolor="#DDDDDD" colspan="5">
						<input name="RE_PR_IO"  id="RE_PR_IO1"   disabled  type="radio" value="In"   <?php if ($listItem[re_pr_io]=='In') echo 'checked="checked"';?>/>In&nbsp;
						<input name="RE_PR_IO"  id="RE_PR_IO2"   disabled  type="radio" value="Out"  <?php if ($listItem[re_pr_io]=='Out') echo 'checked="checked"';?> />Out 
						<input name="RE_PR_IO"  id="RE_PR_IO3"   disabled  type="radio" value="None" <?php if ($listItem[re_pr_io]=='None') echo 'checked="checked"';?> />None&nbsp;&nbsp;
						<input name="re_pr_ax"  id="re_pr_ax"    disabled  type="text"  value="<?php	if ($listItem[re_pr_ax]>0) echo $listItem[re_pr_ax];?>"   size="2" maxlength="4"><br>
						<input name="RE_PR_UD"  id="RE_PR_UD1"   disabled  type="radio" value="Up"   <?php if ($listItem[re_pr_ud]=='Up') echo 'checked="checked"';?>/>Up&nbsp;
						<input name="RE_PR_UD"  id="RE_PR_UD2"   disabled  type="radio" value="Down" <?php if ($listItem[re_pr_ud]=='Down') echo 'checked="checked"';?>/>Down 
						<input name="RE_PR_UD"  id="RE_PR_UD3"   disabled  type="radio" value="None" <?php if ($listItem[re_pr_ud]=='None') echo 'checked="checked"';?>/>None
						<input name="re_pr_ax2" id="re_pr_ax2"   disabled  type="text"  value="<?php	if ($listItem[re_pr_ax2]>0) echo $listItem[re_pr_ax2];?>"   size="2" maxlength="4">
					</td>
                ...
                <tr>
                    ...
                    <input name="re_pd" type="text" class="formField3" id="re_pd"            value="<?php if ($listItem[re_pd]>0)      echo $listItem[re_pd];?>"      size="4" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;
					
					<strong>Near PD:</strong>
                    <input name="re_pd_near" type="text" class="formField3" id="re_pd_near"  value="<?php if ($listItem[re_pd_near]>0) echo $listItem[re_pd_near];?>" size="4" maxlength="4" disabled> &nbsp;&nbsp;&nbsp;
					
					<strong>Height:</strong>
                    <input name="re_height" type="text" class="formField3" id="re_height"    value="<?php if ($listItem[re_height]>0)  echo $listItem[re_height];?>"  size="4" maxlength="4" disabled>
					</td>
                </tr>
                ...
                <tr>
                  ...
                  <td align="center" bgcolor="#DDDDDD">
					  <select name="le_sphere"  id="le_sphere" disabled>
					  <?php
					  $min = -15.75;
					  $max = 14.75;
					  if ($listItem[eye]!="R.E."){
                  ...
                  <td align="center" bgcolor="#DDDDDD">
						  <select name="le_cyl" id="le_cyl" disabled>
						  <?php
						  $min = -8.75;
						  $max = 6.75;
						   if ($listItem[eye]!="R.E."){
                  ...
                  <td align="center" bgcolor="#DDDDDD"><input name="le_axis" type="text" class="formField3" id="le_axis" value="<?php if ($listItem[le_axis]>0) echo $listItem[le_axis];?>"  size="4" maxlength="4" disabled></td>
					
                  <td align="center" bgcolor="#DDDDDD">
					  <select name="le_add" id="le_add" disabled>
						  <?php
						  $max = 4.0;
						  $min = 0;
                ...
                <td align="right" bgcolor="#DDDDDD" colspan="5">
                    <input name="LE_PR_IO" id="LE_PR_IO1"  disabled type="radio" value="In"  <?php if ($listItem[le_pr_io]=='In')   echo 'checked="checked"';?>/>In&nbsp;
  					<input name="LE_PR_IO" id="LE_PR_IO2"  disabled  type="radio" value="Out"  <?php if ($listItem[le_pr_io]=='Out')  echo 'checked="checked"';?>/>Out 
                    <input name="LE_PR_IO" id="LE_PR_IO3"  disabled  type="radio" value="None" <?php if ($listItem[le_pr_io]=='None') echo 'checked="checked"';?>/>None 
					<input name="le_pr_ax" id="le_pr_ax"  disabled  type="text" class="formField3"  value="<?php if ($listItem[le_pr_ax]>0) echo $listItem[le_pr_ax];?>" size="2" maxlength="4"><br>
                   
					<input name="LE_PR_UD"  id="LE_PR_UD1"  disabled  type="radio" value="Up" <?php if ($listItem[le_pr_ud]=='Up') echo 'checked="checked"';?>/>Up&nbsp;
					<input name="LE_PR_UD"  id="LE_PR_UD2"  disabled  type="radio" value="Down" <?php if ($listItem[le_pr_ud]=='Down') echo 'checked="checked"';?>/>Down 
                    <input name="LE_PR_UD"  id="LE_PR_UD3"  disabled  type="radio" value="None" <?php if ($listItem[le_pr_ud]=='None') echo 'checked="checked"';?>/>None&nbsp; 
					<input name="le_pr_ax2" id="le_pr_ax2"  disabled  type="text" class="formField3"  value="<?php if ($listItem[le_pr_ax2]>0) echo $listItem[le_pr_ax2];?>" size="2" docum,enmaxlength="4">
				</td>
                ...
                <tr>
					<td colspan="10" align="left" bgcolor="#FFFFFF">
						<strong>Dist.PD:</strong><input name="le_pd"      type="text" class="formField3" id="le_pd"      size="4" maxlength="4" value="<?php echo $listItem[le_pd];?>"  disabled>&nbsp;&nbsp;&nbsp;
						<strong>Near PD:</strong><input name="le_pd_near" type="text" class="formField3" id="le_pd_near" value="<?php echo $listItem[le_pd_near];?>" size="4" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;
						<strong>Height:</strong> <input name="le_height"  type="text" class="formField3" id="le_height"  size="4" maxlength="4" value="<?php if ($listItem[le_height]>0) echo $listItem[le_height];?>" disabled>
					</td>
                </tr>
            ...
            <tr>
         		<td colspan="10" align="left" bgcolor="#DDDDDD">
					<strong>PT:</strong><input name="PT" type="text" class="formField3" id="PT" value="<?php echo $listItem[PT];?>" size="4" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;
					<strong>PA:</strong><input name="PA" type="text" class="formField3" id="PA" value="<?php echo $listItem[PA];?>" size="4" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;
					<strong>Vertex:</strong><input name="vertex" type="text" class="formField3" id="vertex" value="<?php echo $listItem[vertex];?>" size="4" maxlength="4" disabled>
				</td>
         	 </tr>
        		 ...
        		 <tr>
              	...
              	<tr>
					<td bgcolor="#DDDDDD"  align="left" colspan="10">
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						R.E. <b>CT</b>&nbsp;&nbsp;<input name="RE_CT" type="text"  class="formText" value="<?php echo $listItem[RE_CT];?>" id="RE_CT" size="4" maxlength="6" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					    L.E. <b>CT</b>&nbsp;&nbsp;<input name="LE_CT" type="text" class="formText" value="<?php echo $listItem[LE_CT];?>" id="LE_CT" size="4" maxlength="6" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					
					
						R.E. <b>ET</b>&nbsp;&nbsp;<input name="RE_ET" type="text" class="formText" value="<?php echo $listItem[RE_ET];?>" id="RE_ET" size="4" maxlength="6" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						L.E. <b>ET</b>&nbsp;&nbsp;<input name="LE_ET" type="text" class="formText" value="<?php echo $listItem[LE_ET];?>" id="LE_ET" size="4" maxlength="6" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
					</td>
                </tr>
	...
	?>
			...
			<tr>
				<td colspan="10" align="left" bgcolor="#DDDDDD">
					...
					<input name="special_instructions" type="text" disabled class="formField3" id="special_instructions" value="<?php echo $special_instructions;?>" size="60">			
				</td>
            </tr>
                ...
                <tr>
                	<td colspan="10" align="left" bgcolor="#DDDDDD">
					<b><?php  if ($mylang == 'lang_french'){echo "Note Interne:";}else{echo "Internal Note:";}?></b> <input name="internal_note" type="text" class="formField3" disabled id="internal_note" value="<?php echo $listItem[internal_note];?>" size="40">
			    	</td>
                </tr>
                ...
                <tr>
					<td align="left" bgcolor="#DDDDDD" colspan="10">
						<b>Additional Item:</b>
						<input name="extra_product" type="text" class="formField3" id="extra_product" value="<?php echo $listItem[extra_product];?>" size="30" disabled>
						<b>&nbsp;&nbsp;&nbsp;Amount:$</b>
						<input name="extra_product_price" type="text" class="formField3" <?php if ($manage_additional_discount=='no'){ echo 'disabled="disabled"';}?> id="extra_product_price" 
							   value="<?php echo $listItem[extra_product_price];?>" size="4" disabled>
					</td>
                </tr>
			...
			<tr>
                	 <td colspan="3" align="center" bgcolor="#DDDDDD">
						 ...
						 $edgingItem   = mysqli_fetch_array($edgingResult,MYSQLI_ASSOC);
						 ?>
							 <select name="job_type" id="job_type"  disabled>
					...
					<td colspan="3" align="center" bgcolor="#DDDDDD">
						<b>O.C:</b><input name="optical_center" type="text" class="formField3" id="optical_center" value="<?php echo $listItem[optical_center];?>" size="6" maxlength="6" disabled>
					</td>
                 
					<td colspan="4" bgcolor="#DDDDDD" align="center">
							 <select name="base_curve" class="formText" id="base_curve" disabled>
				...
				<tr>
					<td align="left" colspan="10" bgcolor="#DDDDDD">
						...
						<select name="mirror" id="mirror" disabled>
			...
			<tr>
                ...
                <td align="left" colspan="10" bgcolor="#DDDDDD">
					...
					$tintItem   = mysqli_fetch_array($tintResult,MYSQLI_ASSOC);
					?>
					<select name="tint" id="tint" disabled>
					...
					From <input name="from_perc" id="from_perc" type="text" class="formField3" value="<?php echo $tintItem[from_perc];?>" size="2" maxlength="4" disabled/>&nbsp;% 
					To   <input name="to_perc"   id="to_perc"   type="text" class="formField3" value="<?php echo $tintItem[to_perc];?>"   size="2" maxlength="4" disabled>&nbsp;%  

					<b>Tint Color:</b>

				
						<select name="tint_color" id="tint_color" disabled>
							<option value="None">None</option>
			...
			<tr>
				<td colspan="10"  align="left" bgcolor="#DDDDDD">
					  ...
					  ?>
					  <b>Type:</b>
					<select name="frame_type" id="frame_type" disabled>
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>A:</b> <input name="frame_a" type="text" class="formField3" id="frame_a" value="<?php echo $Frame_A;?>" size="4" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

					<b>B:</b>
					<input name="frame_b" type="text" class="formField3" id="frame_b" value="<?php echo $Frame_B;?>" size="2" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

					 <b>ED:</b>
					<input name="frame_ed" type="text" class="formField3" id="frame_ed" value="<?php echo $Frame_ED ;?>" size="2" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

					<b>DBL:</b>
					<input name="frame_dbl" type="text" class="formField3" id="frame_dbl" value="<?php echo $Frame_DBL;?>" size="2" maxlength="4" disabled>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 <tr>         
	<td colspan="10" align="left" bgcolor="#DDDDDD">
	<b><?php if($mylang == 'lang_french'){echo "Fournisseur";}else{echo "Supplier";}?></b>:<input name="supplier" type="text" id="supplier" class="formField3" value="<?php echo $frameItem[supplier];?>" size="15" disabled/>&nbsp;&nbsp;&nbsp;
	<b><?php if($mylang == 'lang_french'){echo "Model";}else{echo "Frame Modèle";}?></b>:<input type="text" name="temple_model_num" size="15" id="temple_model_num" value="<?php echo $frameItem[temple_model_num];?>" disabled/>&nbsp;&nbsp;&nbsp;
	<b><?php if($mylang == 'lang_french'){echo "Couleur";}else{echo "Color";}?></b>:<input type="text" name="color" id="color" size="10" value="<?php echo $frameItem[color];?>" disabled/>&nbsp;&nbsp;&nbsp;		 
	</td>							
</tr>
		...
		<?php
			...
			//echo $queryDateProcessedOriginal;
			$resultDateProcessedOriginal= mysqli_query($con,$queryDateProcessedOriginal)		or die  ('I cannot select items because 4: ' . $queryDateProcessedOriginal . mysqli_error($con));
			...
			<tr>
			 ...
			 <tr><td colspan="10">&nbsp;</td></tr>	 
                ...
                <tr align="center">
                    ...
                    <input name="validate_pwd" type="button" id="validate_pwd" autocomplete="off" class="formField3" value="Valider">
              ...
              <tr>
			  	  <td colspan="10" align="center" bgcolor="#AAAAAA">
					  <?php //<input name="Submitbtn" type="submit" id="Submitbtn" class="formField3" value="Update" disabled> ?>
					  <input name="Submitbtn" type="submit" id="Submitbtn" class="formField3" value="Update" disabled onclick="ActiverChampEtSoumettreForm()" > 
