...
FROM orders, accounts A
WHERE orders.user_id = A.user_id
AND orders.order_status NOT IN ('cancelled', 'filled', 'basket')
AND orders.special_instructions like '%base%'
AND order_date_processed = '$datecomplete'
OR
orders.user_id = A.user_id
AND orders.order_status NOT IN ('cancelled', 'filled', 'basket')
AND orders.special_instructions like '%courbure%'
AND order_date_processed = '$datecomplete'
OR
orders.user_id = A.user_id
AND orders.order_status NOT IN ('cancelled', 'filled', 'basket')
AND orders.special_instructions like '%curve%'
AND order_date_processed = '$datecomplete'
...
$subject= "Commandes avec Bases Speciales: " . $datecomplete;
...
//Log email
	...
	if($response){ 
		log_email("REPORT: Special Base Report",$EmailEnvoyerA,"SENT",$_SERVER['HTTP_USER_AGENT']);
		echo 'Logged: sucess';
    }else{
		log_email("REPORT: Special Base Report",$EmailEnvoyerA,"FAILED",$_SERVER['HTTP_USER_AGENT']);
		echo 'Logged: failed';
	}	
