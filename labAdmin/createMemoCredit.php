...
if($_GET["order_num"]!=""){
		...
		$orderQuery="select order_num, user_id, order_status, additional_dsc, discount_type, extra_product, extra_product_price, order_date_processed, order_patient_first, order_patient_last, patient_ref_num, order_total from orders WHERE order_num='$_GET[order_num]' AND lab='$lab_pkey' limit 1"; //get order's user id and additional discount
	}
	elseif(($_POST["order_num"]!="")&&($_POST["patient_ref_num"]!="")){
		...
		$orderQuery="select order_num, user_id, order_status, additional_dsc, discount_type, extra_product, extra_product_price, order_date_processed, order_patient_first, order_patient_last, patient_ref_num, order_total from orders WHERE order_num='$_POST[order_num]' AND lab='$lab_pkey' AND (patient_ref_num like '$patient_ref_num' OR order_patient_first like '$patient_ref_num' OR order_patient_last like '$patient_ref_num') limit 1"; //get order's user id and additional discount
	}
	elseif(($_POST["order_num"]=="")&&($_POST["patient_ref_num"]!="")){
		$patient_ref_num="%".$_POST["patient_ref_num"]."%";
		$_SESSION["patient_ref_num"]=$patient_ref_num;
		$_SESSION["order_num"]="";
		$orderQuery="select order_num, user_id, order_status, additional_dsc, discount_type, extra_product, extra_product_price, order_date_processed, order_patient_first, order_patient_last, patient_ref_num, order_total from orders WHERE lab='$lab_pkey' AND (patient_ref_num like '$patient_ref_num' OR order_patient_first like '$patient_ref_num' OR order_patient_last like '$patient_ref_num') group by order_num"; //get order's user id and additional discount
	}
	elseif(($_POST["order_num"]!="")&&($_POST["patient_ref_num"]=="")){
		$_SESSION["patient_ref_num"]="";
		$_SESSION["order_num"]=$_POST["order_num"];
		$orderQuery="select order_num, user_id, order_status, additional_dsc, discount_type, extra_product, extra_product_price, order_date_processed, order_patient_first, order_patient_last, patient_ref_num, order_total from orders WHERE order_num='$_POST[order_num]' AND lab='$lab_pkey' limit 1"; //get order's user id and additional discount
	}
	elseif(($_POST["order_num"]=="")&&($_POST["patient_ref_num"]=="")){
		$_SESSION["patient_ref_num"]="";
		$_SESSION["order_num"]=="";
	...
	if($orderCount==1){
		...
		$lastMemoQuery="select mcred_memo_num from memo_credits WHERE mcred_order_num='$orderData[order_num]' ORDER BY mcred_memo_num DESC limit 1"; //get the most recent memo credit for this order number
		...
		$memoQuery="select * from memo_credits WHERE mcred_order_num='$orderData[order_num]' ORDER BY mcred_memo_num"; //get all memo credits for this order number
...
var myCalendar;
function doOnLoad() {
    myCalendar = new dhtmlXCalendarObject(["mcred_date"]);
	setFrom();
}

function setFrom() {
    myCalendar.setSensitiveRange("<?php echo $todayjs; ?>", null);
    }
</script>
...
function formCheck(formobj){
	...
	if ((formobj.mcred_memo_code.value=='6.4') && (formobj.other_detail.value=='')){
	alert("Since you selected Other as the credit reason, please fill in the field to explain the 'Other'");
	return false;
	}	
	
		
	var PtstoSubstract = formobj.optipoints_to_substract.value;
	var SoldePts 	   = formobj.point_balance.value
	
	
	if (PtstoSubstract  != 0)
	{
		var resultatSolde = SoldePts -  PtstoSubstract;	
		if (resultatSolde < 0){
			alert("You cannot substract more optipoints than the customer Optipoints balance.");
			return false;
		}	
	 }
