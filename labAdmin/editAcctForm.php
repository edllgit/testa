...
$sales_reps = mysqli_query($con,$query_sales_reps, $directlens) or die(mysqli_error($con));
$row_sales_reps = mysqli_fetch_array($sales_reps,MYSQLI_ASSOC);
$totalRows_sales_reps = mysqli_num_rows($sales_reps);
...
?>
				    ...
				    <?php
} while ($row_sales_reps = mysqli_fetch_array($sales_reps,MYSQLI_ASSOC));
  $rows = mysqli_num_rows($sales_reps);
  if($rows > 0) {
      mysqli_data_seek($sales_reps, 0);
	  $row_sales_reps = mysqli_fetch_array($sales_reps,MYSQLI_ASSOC);
  }
?>
				    </select></td>
	...
	$innovative_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query) or die ("Could not find lab");
	$infocus_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query)		or die ("Could not find lab");
	$precision_vp_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query) or die ("Could not find lab");
	$visionpropoly_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query)		or die ("Could not find lab");
	$generation_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query) or die ("Could not find lab");
	$truehd_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query)		or die ("Could not find lab");
	$easy_fit_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
		//echo "<b>$truehd_lab[lab_name]</b>";
	?></td>
				  <td align="left" bgcolor="#FFFFFF" >&nbsp;</td>
	...
	$result=mysqli_query($con,$query) 		or die ("Could not find lab");
	$main_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
	...
	$result=mysqli_query($con,$query)		or die ("Could not find lab");
	$other_lab=mysqli_fetch_array($result,MYSQLI_ASSOC);
		echo "n/a";
...
$stockResult=mysqli_query($con,$stockQuery)	or die ($adm_nostock_txt);
$tableCounter=0;
echo "<form action=\"getAccount.php\" method=\"post\" name=\"discountForm\"><tr bgcolor=\"#DDDDDD\">";
while ($stockData=mysqli_fetch_array($stockResult,MYSQLI_ASSOC)){
	...
	$dscResult=mysqli_query($con,$dscQuery)		or die ($adm_nostock_txt);
	$dscCount=mysqli_num_rows($dscResult);
	if($dscCount!=0){
		$dscData=mysqli_fetch_array($dscResult,MYSQLI_ASSOC);
