...
if ($_GET[pkey] != ""){
	$pkey = $_GET[pkey];
	$heading="SAFETY PRODUCT FORM";
}

if ($_POST[editProduct] == "Update Product"){
	$pkey = $_POST[pkey];
	edit_safety_exclusive_product($pkey);
	$heading="SAFETY PRODUCT FORM <font color=\"#FF0000\">PRODUCT EDITED</font>";
}
if (!$_POST[createProduct] == "Create Product"){
	$query="SELECT * FROM safety_exclusive where primary_key = '$pkey'";
...
<link href="admin.css" rel="stylesheet" type="text/css" />
  ...
  <form name="form4" method="post" action="update_exclusive_product_safety.php"  enctype="multipart/form-data">
  ...
  <?php
  $query="SELECT collection_name FROM liste_collection_info WHERE collection_name IN ('Safety Versano','Safety STC','Safety HKO','Safety Swiss') ORDER BY collection_name asc"; /* select all openings */
...
$usercount=mysqli_num_rows($result);
 ...
 echo "$name</option>";}?>
  ...
  <?php
  $query="SELECT manufacturer FROM safety_exclusive group by manufacturer asc"; /* select all openings */
$result=mysqli_query($con,$query)		or die ("Could not select items");
$usercount=mysqli_num_rows($result);
 while ($listItem=mysqli_fetch_array($result,MYSQLI_ASSOC)){
...
<?php  
$queryLens = "SELECT lens_category FROM safety_exclusive where primary_key = $pkey";
$resultLens=mysqli_query($con,$queryLens)		or die ("Could not select items". mysqli_error($con).$queryLens);
...
<option  <?php if ($lenscatego == '') 		 echo 'selected'; ?> value="">None</option>
  ...
  <?php
  $query="SELECT coating FROM safety_exclusive group by coating asc"; /* select all openings */
$result=mysqli_query($con,$query)		or die ("Could not select items");
$usercount=mysqli_num_rows($result);
 while ($listItem=mysqli_fetch_array($result,MYSQLI_ASSOC)){
 ...
 echo "$name</option>";}?>
      ...
      <tr bgcolor="#DDDDDD">
        ...
        Discounted:</font></td>
        <td align="left" bgcolor="#FFFFFF"><font size="1">$
            <input name="price_discounted" type="text" class="formText" id="price_discounted" value="<?php echo "$productData[price_discounted]"; ?>" size="6" />
        </font></td>
        <td align="right" nowrap="nowrap" colspan="2" bgcolor="#FFFFFF"><font size="1" face="Arial, Helvetica, sans-serif">&nbsp;</font></td>
  ...
  <?php
  $query="SELECT photo FROM safety_exclusive group by photo asc"; /* select all openings */
$result=mysqli_query($con,$query)		or die ("Could not select items");
$usercount=mysqli_num_rows($result);
 while ($listItem=mysqli_fetch_array($result,MYSQLI_ASSOC)){
 ...
 echo "$name</option>";}?>
      ...
      <tr bgcolor="#DDDDDD">
        <td align="right" bgcolor="#DDDDDD"><font size="1" face="Arial, Helvetica, sans-serif">Price
              Interco:</font></td>
        <td align="left" bgcolor="#DDDDDD"><font size="1">$
          <input name="price_interco" type="text" class="formText" id="price_interco" value="<?php echo "$productData[price_interco]"; ?>" size="6" />
        </font></td>
