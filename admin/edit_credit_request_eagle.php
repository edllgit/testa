...
if ($_POST[mcred_memo_num] <> ''){
	 $mcred_memo_num 	  = mysql_escape_string($_POST[mcred_memo_num]);
	 $order_num      	  = substr($mcred_memo_num,1,7);	
	
	 $queryRequestDetail  = "SELECT * FROM memo_credits_temp WHERE mcred_memo_num = '$mcred_memo_num' ";
	 $resultRequestDetail = mysql_query($queryRequestDetail)		or die ('Could not check already credited because: ' . mysql_error());
	 $DataCredit	      = mysql_fetch_array($resultRequestDetail); 
	 ...
	 $resultAccount 	  = mysql_query($queryAccount)		or die ('Could not check already credited because: ' . mysql_error());
	 ...
	 $acces_id = $_SESSION["access_admin_id"];
		 ...
		 $resultUpdateDate = mysql_query($queryUpdateDate)		or die ('Could not update mcred_date because: ' . mysql_error());
	...
	VALUES ('$mcred_memo_num',   $order_num,    'Credit date updated from <b>$DataCredit[mcred_date]</b> to  <b>$_POST[mcred_date]</b>', 'Date du crédit modifié de <b>$DataCredit[mcred_date]</b> a <b>$_POST[mcred_date]</b>',     '$datecomplete',        'manual',     '$ip',     $acces_id)";
		 $resultUpdateDate = mysql_query($queryHistory)		or die ('Could not update mcred_date because: ' . mysql_error());
		 }
		 ...
		 $resultUpdateDate = mysql_query($queryUpdateDate)		or die ('Could not update mcred_abs_amount because: ' . mysql_error());
	...
	VALUES ('$mcred_memo_num',   $order_num,    'Credit amount updated from <b>$DataCredit[mcred_abs_amount]$</b> to  <b>$_POST[mcred_abs_amount]$</b>', 'Montant du crédit mis à jour de <b>$DataCredit[mcred_abs_amount]$</b> a  <b>$_POST[mcred_abs_amount]$</b>',    '$datecomplete',        'manual',     '$ip',     $acces_id)";
		 $resultUpdateDate = mysql_query($queryHistory)		or die ('Could not update mcred_abs_amount because: ' . mysql_error());
		 }
		 ...
		 $resultUpdateDate = mysql_query($queryUpdateDate)		or die ('Could not update mcred_memo_code because: ' . mysql_error());
		 $queryHistory = "INSERT INTO  memo_credits_status_history 
		   (mcred_memo_num,       order_num,      request_status, request_status_fr,                                                               update_time,        update_type,  update_ip,   access_id)
	VALUES ('$mcred_memo_num',   $order_num,    'Credit reason updated from <b>$DataCredit[mcred_memo_code]</b> to  <b>$_POST[mcred_memo_code]</b>',  'Raison du crédit mis a jour de <b>$DataCredit[mcred_memo_code]</b> a <b>$_POST[mcred_memo_code]</b>',    '$datecomplete',        'manual',     '$ip',     $acces_id)";
		 $resultUpdateDate = mysql_query($queryHistory)		or die ('Could not update mcred_memo_code because: ' . mysql_error());
		 }
...
?>
<form id="redirect" action="edit_credit_request_eagle.php" method="post">
...
<body>
<form name="update_credit_request" id="update_credit_request" method="post" action="edit_credit_request_eagle.php">
...
<p><a href="credit_status_update_detail_eagle.php?mcred_memo_num=<?php echo $mcred_memo_num ;?>">Back to update status page</a></p>
</td>
	  </tr>
</table>
  <p>&nbsp;</p>
