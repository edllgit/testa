...
foreach( $_POST[UpdateOrderNum] as $the_order_num ){	
	...
	if (strlen($the_order_num)==5)
	{
		...
		//WE insert this info regarding the frame sent to KNR in the status_history
		$queryStatus="INSERT INTO status_history (order_num, order_status, update_type, update_time,update_ip, access_id, update_ip2, provient_de, browser  ) 
										  VALUES($the_order_num,'Frame sent to KNR/Monture envoyé à KNR','Admin fast shipping page','$datecomplete','$ip',$acces_id, '$ip2', '$provient_de','$browser')";
		
		//echo '<br>'.$queryStatus . '<br>';
					  						  		
		$resultStatus=mysqli_query($con,$queryStatus)		or die ('Could not insert because: ' . mysqli_error($con));
		
		//We update the frame_sent_knr FIELD
		$queryUpdate	 = "UPDATE orders SET frame_sent_knr = '$datecomplete' WHERE order_num = $the_order_num";
		//echo '<br>'.$queryUpdate.'<br>';
		
		$UpdateDetails  .= "<br>Order #$the_order_num Has been saved as  Frame sent to KNR";
...
<html>
<head> 
<title>Send Frames to KNR</title>
  ...
  <table border="0" cellspacing="0" cellpadding="0" width="95%">
  	<tr valign="top">
		...
		?></td>
  		...
  		<td width="1" bgcolor="#FFFFFF">&nbsp;</td>
  		<td width="75%">
<form  method="post" name="verification" id="verification" action="send_hbc_frame_knr.php">
            <table width="100%" border="0" cellpadding="2" cellspacing="0" class="formField">
            	<tr bgcolor="#000000">
            		<td align="center" colspan="4"><b><font color="#FFFFFF" size="1" face="Helvetica, sans-serif, Arial">Note frames as Sent to KNR</font></b></td>
            	</tr>
...
<form  method="post" name="update_status" id="update_status" action="send_hbc_frame_knr.php">
			<?php 
			if ($rptQuery!=""){
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
 ...
 if ($OrderAbleToUpdate > 0){
	...
	<td align=\"center\" colspan=\"11\" nowrap=\"nowrap\">";
				  ...
				  if ($OrderAbleToUpdate > 0){
				  echo "<input name=\"update_status\" style=\"font-size:14px;\"  type=\"submit\" id=\"rpt_search\" value=\"Mark these orders as Frames sent to KNR\" class=\"formField\"></tr>";
				  }else{
				  echo "<input name=\"update_status\" style=\"font-size:14px;\"  type=\"submit\" id=\"rpt_search\" disabled=\"disabled\" value=\"Mark these orders as Frames sent to KNR\" class=\"formField\"></tr>";
				  }
