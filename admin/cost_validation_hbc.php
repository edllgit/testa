...
if($_POST[rpt_search]=="Verifier/Verify")
{
	...
	//Valider les numéros de commandes passé, longeur doit etre de 5, doit etre numeric
	$errorMessage = '';
	$PassValidation = true;
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			//Si la longeur est > a 5 et qu'il n'y a pas de virgule, l'usager ne sait surement pas qu'il doit entrer des virgule entre chaque numéro
  ...
  <table border="0" cellspacing="0" cellpadding="0" width="95%">
  	<tr valign="top">
		...
		?></td>
  		...
  		<td width="1" bgcolor="#FFFFFF">&nbsp;</td>
  		<td width="85%">
<form  method="post" name="verification" id="verification" action="cost_validation_hbc.php">
            <table width="100%" border="0" cellpadding="2" cellspacing="0" class="formField">
            	<tr bgcolor="#000000">
            		<td align="center" colspan="4"><b><font color="#FFFFFF" size="1" face="Helvetica, sans-serif, Arial">HBC: Cost validation tool</font></b></td>
            	</tr>
...
<form  method="post" name="update_status" id="update_status" action="cost_validation_hbc.php">
			<?php 
			if ($rptQuery!=""){
...
<tr bgcolor=\"#000000\"></tr>";
	  echo "<tr>
                ...
                <th align=\"center\"><strong>Lenses</strong></th>
				<th align=\"center\"><strong>Lenses Cost</strong></th>
				<th align=\"center\"><strong>Edging Type/Cost</strong></th>
				<th align=\"center\"><strong>Frame handling fee</strong></th>
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
		...
		$queryEdging  	= "SELECT  frame_type FROM extra_product_orders WHERE order_num=$listItem[order_num] AND category='Edging'";
		$resultEdging 	= mysqli_query($con,$queryEdging)		or die ('Could not insert because: ' . mysqli_error($con));
		$DataEdging   	= mysqli_fetch_array($resultEdging,MYSQLI_ASSOC);
		$FrameType 		= $DataEdging[frame_type];
		
		//echo '<br>FRAME TYPE:' . $FrameType ;
	
		switch($FrameType){//Prix à la paire
			case 'Drill and Notch':	$PrixEdging = 4.30;   break;	
			case 'Plastic':			$PrixEdging = 2.50;   break;	
			case 'Metal':			$PrixEdging = 2.50;   break;		
			case 'Metal Groove':	$PrixEdging = 3.10;   break;	
			case 'Nylon Groove':	$PrixEdging = 3.10;   break;			
		}//End Switch
	
		$PrixEdging = money_format('%.2n',$PrixEdging);	
	
	
		$queryOther 	= "SELECT  * FROM extra_product_orders WHERE order_num=$listItem[order_num] AND category  IN ('Prism','Cylinder Over Range','Tint','Iimpact','Mirror')";
		$resultOther 	= mysqli_query($con,$queryOther)		or die ('Could not insert because: ' . mysqli_error($con));
		$ContenuOther   = "";
	
	
		while ($DataOther  	= mysqli_fetch_array($resultOther,MYSQLI_ASSOC)){
			
			//Calcul du nombre de prisme dans la commande pour calculer leurs prix coutants
			$queryPrism 	= "SELECT re_pr_ax,	le_pr_ax, re_pr_ax2, le_pr_ax2 FROM orders WHERE order_num= $listItem[order_num]";
			$resultPrism 	= mysqli_query($con,$queryPrism)		or die ('Could not insert because: ' . mysqli_error($con));
			$DataPrism		= mysqli_fetch_array($resultPrism,MYSQLI_ASSOC);
			$NombredePrisme = 0;//Initialiser le compteur
			if ($DataPrism[re_pr_ax]<>''){ $NombredePrisme  = $NombredePrisme +1;}//End IF
			if ($DataPrism[re_pr_ax2]<>''){$NombredePrisme  = $NombredePrisme +1;}//End IF
			if ($DataPrism[le_pr_ax]<>''){ $NombredePrisme  = $NombredePrisme +1;}//End IF
			if ($DataPrism[le_pr_ax2]<>''){$NombredePrisme  = $NombredePrisme +1;}//End IF
			$CoutPrisme = 2.2 * $NombredePrisme ;
			$CoutPrisme=money_format('%.2n',$CoutPrisme);
			//echo '<br>Cout prisme: '.$CoutPrisme ;//Fonctionne

				switch ($DataOther[category]){
					case 'Prism':  $ContenuOther = "Prism: $CoutPrisme$";	  break;

					//case 'Tint':  	$Todo='Aller chercher le type de teinte dans la commande';  	 break;		
					//case 'Iimpact': $Todo='Aller chercher le nombre de prisme(s) dans la commande';  break;		
					//case 'Mirror':  $Todo='Aller chercher le nombre de prisme(s) dans la commande';  break;	
				}//End Switch
		...
		$TotalinludingEdging = $DataProduct[cost_us] + $PrixEdging + 0.50 + $CoutPrisme ;
		$TotalinludingEdging=money_format('%.2n',$TotalinludingEdging);
	
	
		$TotalCostUSD = $TotalCostUSD + $TotalinludingEdging;
		...
		<td style=\"font-size:13px;\" align=\"center\">$listItem[order_num]</td>
		<td style=\"font-size:13px;\" align=\"center\">$DataProduct[product_code]</td>
		<td style=\"font-size:13px;\" align=\"center\">$DataProduct[product_name]</td>
		<td style=\"font-size:13px;\" align=\"center\">$DataProduct[cost_us]$</td>
		<td style=\"font-size:13px;\" align=\"center\">$FrameType / $PrixEdging$</td>
		<td style=\"font-size:13px;\" align=\"center\">0.50$</td>
		<td style=\"font-size:13px;\" align=\"center\">$ContenuOther</td>
		<td style=\"font-size:13px;\" align=\"center\">$TotalinludingEdging$</td>
		</tr>";

}//END WHILE

	
	//Afficher le total dans le bas des factures affichées
	$TotalCostUSD = money_format('%.2n',$TotalCostUSD);		
	echo "<tr><td colspan=\"6\" align=\"right\" style=\"font-size:16px;\"><b>Total USD:</b></td><td colspan=\"3\" align=\"center\" style=\"font-size:16px;\"><b>$TotalCostUSD$</b></td></tr>"	;
