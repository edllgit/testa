...
if ($_POST[editAcct] == "Edit Account"){
	$pkey = $_POST[pkey];
	writemycollections($pkey);
	addAccountStockCollections($pkey);// add selected stock collections for user to accounts_stock_collections table
	edit_account($pkey);
}
if ($_POST[deleteAcct] == "Delete Account"){//NOT USED
	$pkey = $_POST[pkey];
	delete_account($pkey);
	addAccountStockCollections($pkey);// add selected stock collections for user to accounts_stock_collections table
}
...
$acctResult=mysqli_query($con,$query)	or die ("Could not find account" . mysqli_error($con)." query=".$query);
...
function formCheck(formobj){
	...
	var fieldRequired = Array("title", "first_name", "last_name", "company", "bill_address1", "password");
	// field description to appear in the dialog box
	var fieldDescription = Array("Title", "First Name", "Last Name", "Company", "Billing Address 1", "Password");
	// dialog message
...
<body>
  <table border="0" cellspacing="0" cellpadding="0" width="95%">
  	<tr valign="top">
		...
		elseif ($_POST[editAcct] == "Edit Account"){
