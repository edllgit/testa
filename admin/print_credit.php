...
<table width="650" border="0"cellpadding="3" cellspacing="0" align="center"><tr>
   ...
   <?php 
	$queryLogo ="SELECT  logo_file FROM labs WHERE primary_key = (SELECT distinct lab FROM orders WHERE order_num = " .  substr($_REQUEST[memo_num],1,7) . ")";
	//echo '<br>'. $queryLogo;
	...
	$queryPrescriptLab  = "SELECT lab FROM orders WHERE order_num = " .  substr($_REQUEST[memo_num],1,7);
	$resultPrescriptLab = mysqli_query($con,$queryPrescriptLab)	or die  ('I cannot select items because 1 : ' . mysqli_error());
