...
if($_POST[rpt_search]=="Verify")
{
	...
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			WHERE orders.order_num IN ($_POST[order_num]) GROUP BY order_num";
			//echo '<br><br>'. $rptQuery;
			}else{
...
foreach( $_POST[UpdateOrderNum] as $the_order_num ){	
	...
	//	echo '<br>$InsertQuery'.$InsertQuery.'<br>';
		...
		//echo '<br>InsertQueryHistory'.$InsertQueryHistory.'<br>';
...
<form  method="post" name="verification" id="verification" action="credit_reception_hbc.php">
            <table width="100%" border="0" cellpadding="2" cellspacing="0" class="formField">
            	<tr bgcolor="#000000">
            		<td align="center" colspan="4"><b><font color="#FFFFFF" size="1" face="Helvetica, sans-serif, Arial">HBC Credit Request/Reception</font></b></td>
            	</tr>
				...
				<tr bgcolor="#DDDDDD">
					<td align="center" valign="middle" nowrap bgcolor="#DDDDDD" ><div style="font-size:14px;" align="center">
                        ...
                        <div><strong>1-</strong> Please enter your HBC order numbers separated by a comma <br>(<strong>IMPORTANT:</strong> After the last order number, make sure there are no extra spaces or break line)</div>					<textarea cols="20" name="order_num" style="font-size:16px;"  rows="10" id="order_num" class="formField"><?php if($_POST[rpt_search]=="Verify"){
...
<form  method="post" name="update_status" id="update_status" action="credit_reception_hbc.php">
			<?php 
			if ($rptQuery!=""){
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
			...
			$ship_date=$listItem[order_date_shipped];
				...
				switch($listItem["order_status"]){
						...
						case 'waiting for frame ho/supplier':	$list_order_status = "Waiting for Head Office/Supplier";	break;
