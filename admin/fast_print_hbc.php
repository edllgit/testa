...
if($_POST[rpt_search]=="Verify")
{
	...
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			WHERE orders.order_num IN ($_POST[order_num]) GROUP BY order_num ORDER BY orders.order_num";
...
foreach( $_POST[UpdateOrderNum] as $the_order_num ){	
	...
	if (strlen($the_order_num)==5)
	{
		...
		//We update the tray number
		$queryUpdate	 = "UPDATE orders SET tray_num = '$TrayValue' WHERE order_num = $the_order_num";
		//echo '<br>'. $queryUpdate;
  ...
  <table border="0" cellspacing="0" cellpadding="0" width="95%">
  	<tr valign="top">
		...
		?></td>
  		...
  		<td width="1" bgcolor="#FFFFFF">&nbsp;</td>
  		<td width="75%">
<form  method="post" name="verification" id="verification" action="fast_print_hbc.php">
...
<form  method="post" name="update_status" id="update_status" action="fast_print_hbc.php">
			<?php 
			if ($rptQuery!=""){
...
<td align=\"center\">Check all<br><input name=\"UpdateOrderNum[]\"  title=\"Check all orders\" onclick='checkedAll();' alt=\"Check all orders\" id=\"UpdateOrderNum[]\"  value=\"$listItem[order_num]\"  type=\"checkbox\"/></td>
				...
				<td align=\"center\"><strong>Frame Color</strong></td>
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
		  ...
		  $queryMonture = "SELECT supplier, temple_model_num, color FROM extra_product_orders WHERE order_num = $listItem[order_num] AND category='Frame'";
		  ...
		  $DataMonture=mysqli_fetch_array($resultMonture,MYSQLI_ASSOC);
			...
			if ($list_order_status=='Cancelled'){
				 echo "<td  bgcolor=\"#FF0000\" style=\"font-size:16px;\"  align=\"center\">&nbsp;</td>
					   ...
					   <td  bgcolor=\"#FF0000\" style=\"font-size:16px;\"  align=\"center\"><input disabled type=\"text\" name=\"tray_num/$listItem[order_num]\" id=\"tray_num/$listItem[order_num]\" value=\"$listItem[tray_num]\"/></td>
					   ...
					   <td  bgcolor=\"#FF0000\" style=\"font-size:16px;\"  align=\"center\">$DataMonture[color]</td>
			...
			$OrderAbleToUpdate +=1;
			 echo "<td align=\"center\"><strong><input name=\"UpdateOrderNum[]\"  id=\"UpdateOrderNum[]\"  alt=\"Check this order to update the status\" checked=\"checked\" value=\"$listItem[order_num]\"  type=\"checkbox\"/></strong></td>                   <td style=\"font-size:16px;\"  align=\"center\">$listItem[order_num]</td> 
			       <td style=\"font-size:16px;\"  align=\"center\"><input type=\"text\"  name=\"tray_num/$listItem[order_num]\"  id=\"tray_num/$listItem[order_num]\" value=\"$listItem[tray_num]\"/> </td>
			       <td style=\"font-size:16px;\"  align=\"center\">$order_date</td>
				   <td style=\"font-size:16px;\"  align=\"center\">$DataMonture[temple_model_num]</td>
				   <td style=\"font-size:16px;\"  align=\"center\">$DataMonture[color]</td>
 ...
 if ($OrderAbleToUpdate > 0){
	...
	<td align=\"center\" colspan=\"11\" nowrap=\"nowrap\">";
				  ...
				  if ($OrderAbleToUpdate > 0){
				  echo "<input name=\"update_status\" style=\"font-size:14px;\"  type=\"submit\" id=\"rpt_search\" value=\"Save and Print checked orders\" class=\"formField\"></tr>";
				  }else{
...
All these orders are either cancelled , so their status cannot be updated
</div>';
?>
