...
$mcred_memo_num = mysql_escape_string($_POST[mcred_order_num]);
$order_num      = substr($mcred_memo_num,1,7);			
}

$queryCreditFinished   = "SELECT mcred_approbation FROM memo_credits_temp WHERE mcred_memo_num = '$mcred_memo_num'";
$resultCreditFinished  = mysql_query($queryCreditFinished)		or die ('Could not check already credited because: ' . mysql_error());
$DataCreditFinished    = mysql_fetch_array($resultCreditFinished);
...
$resultOrder  = mysql_query($queryOrder)		or die ('Could not check already credited because: ' . mysql_error());
$DataOrder    = mysql_fetch_array($resultOrder);

$queryCredit   = "SELECT * FROM memo_credits_temp WHERE  mcred_memo_num = '$mcred_memo_num'";
$resultCredit  = mysql_query($queryCredit)		or die ('Could not check already credited because: ' . mysql_error());
$DataCredit    = mysql_fetch_array($resultCredit);

$queryCreditDejaEmis   = "SELECT * FROM memo_credits WHERE  mcred_order_num = $order_num";
$resultCreditDejaEmis  = mysql_query($queryCreditDejaEmis)		or die ('Could not check already credited because: ' . mysql_error());
$nbrResult             = mysql_num_rows($resultCreditDejaEmis);

$queryCreditHistory    = "SELECT * FROM memo_credits_status_history  WHERE  mcred_memo_num = '$mcred_memo_num' order by update_time";
$resultCreditHistory   = mysql_query($queryCreditHistory)		or die ('Could not check already credited because: ' . mysql_error());

$querySoldePoints      = "SELECT  lnc_reward_points   FROM accounts WHERE accounts.user_id='$DataOrder[user_id]' ";
$resultSoldePoints     = mysql_query($querySoldePoints)		or die ('Could not check already credited 2 because: ' . mysql_error());
$DataSoldePoints       = mysql_fetch_array($resultSoldePoints);
...
$resultMemoCode        = mysql_query($queryMemoCode)		or die ('Could not check already credited 2 because: ' . mysql_error());
...
<table width="85%" border="1" cellpadding="2" cellspacing="0" class="formField">              
   ...
   <?php while ($DataCreditDejaEmis = mysql_fetch_array($resultCreditDejaEmis)){   ?>
            <tr>
...
<table width="85%" border="1" cellpadding="2" cellspacing="0" class="formField">  
<form  method="post" name="edit_credit_request" id="edit_credit_request" method="post" action="edit_credit_request_eagle.php">                
...
<form  method="post" name="approve_credit" id="approve_credit" method="post" action="approve_credit_eagle.php">       
<input type="hidden" name="mcred_memo_num" id="mcred_memo_num" value="<?php echo $DataCredit[mcred_memo_num]; ?>" />
...
<form  method="post" name="refuse_credit" id="refuse_credit" method="post" action="refuse_credit_eagle.php">       
<input type="hidden" name="mcred_memo_num" id="mcred_memo_num" value="<?php echo $DataCredit[mcred_memo_num]; ?>" />
...
<select <?php if ($erreur) echo 'disabled';  ?> <?php if ($EmpecherModification) echo 'disabled';?> id="refused_reason" name="refused_reason" class="formField">
				<?php $codeQuery="SELECT * FROM credit_refuse_reasons"; //get lab's memo codes
					  $codeResult=mysql_query($codeQuery)	or die  ('I cannot select items because: ' . mysql_error());
...
<table width="85%" border="1" cellpadding="2" cellspacing="0" class="formField">              
   ...
   <?php while ($DataCreditHistory = mysql_fetch_array($resultCreditHistory)){   ?>
            <tr>
...
<br><br><br><p align=\"center\"><a href="credit_search_eagle.php">Back to credit search</a></p>
