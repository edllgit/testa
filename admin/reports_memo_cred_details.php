...
$datecomplete = $anneeetmois  . '-' . $jour ;
 ...
 if ($mylang == 'lang_french'){
		...
		$heading="Detailed Memo credits Usage report";	
 }
...
<form  method="post" name="goto_date" id="goto_date" action="reports_memo_cred_details.php">
		...
		?>
			...
			<select name="account" id="account" class="formField">
				...
				<?php
	$query="select primary_key, company, last_name, first_name from accounts where  approved='approved' order by company, last_name";
...
if ($_POST[account] <> ''){
$queryClient = "SELECT main_lab FROM  accounts WHERE  accounts.primary_key =  $_POST[account] LIMIT 0,1";
$rptClient=mysql_query($queryClient) 	or die  ('I cannot select items because 1: ' . mysql_error());
$listClient=mysql_fetch_array($rptClient);
}


if ($listClient[main_lab] <> ''){
$queryAccount = "SELECT distinct accounts.*, memo_credits.*, memo_codes.mc_description  FROM  accounts,  memo_credits, memo_codes
WHERE memo_codes.mc_lab = '$listClient[main_lab]' 
...
AND accounts.main_lab =". $listClient[main_lab];
...
$rptAccount=mysql_query($queryAccount) 	or die  ('I cannot select items because 2: ' . mysql_error());

	echo "<tr  align=\"left\"><th  align=\"left\">";
	if ($mylang == 'lang_french'){
	echo  'No de commande:</th><th  align=\"left\">Raison</th><th  align=\"left\">Compte</th><th  align=\"left\">Date</th><th  align=\"left\">Montant</th><th  align=\"left\">Lab</th>';
	}else {
	echo  'Order Number:</th><th  align=\"left\">Reason</th><th  align=\"left\">Account</th><th  align=\"left\">Date</th><th  align=\"left\">Amount</th><th  align=\"left\">Lab</th>';
	}
	echo "</tr>";
...
if ($listAccount['mcred_order_num'] <> ""){
$queryLab = "SELECT lab_name from labs WHERE primary_key = (SELECT lab from orders where order_num = " . $listAccount['mcred_order_num']. " LIMIT 0,1 )";
$rptLab=mysql_query($queryLab) 	or die  ('I cannot select items because 3: ' . mysql_error());
$DataLab=mysql_fetch_array($rptLab);
$labName = $DataLab[lab_name];
}

$totalMemoCredit = $totalMemoCredit+$listAccount['mcred_abs_amount'] ;
		...
		echo "<td  align=\"left\">" 	. $listAccount['mcred_abs_amount']  . "$</td>";
		echo "<td  align=\"left\">" 	. $labName  . "</td></tr>" ;


}//End while listAccount 
echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td><b>&nbsp;</b></td><td><b>---------</b></td></tr>";
echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td><b>Total:</b></td><td><b>$totalMemoCredit$</b></td></tr>";
echo "</table>";
