	...
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			WHERE orders.order_num IN ($_POST[order_num]) AND  
			 memo_credits_temp.mcred_order_num = orders.order_num";
			//echo '<br><br>'. $rptQuery;
			}else{
...
<form  method="post" name="update_status" id="update_status" action="credit_status_update_detail_eagle.php">
			<?php 
			if ($rptQuery!=""){
...
<br><br> <div style="font-size:14px;" align="center">Select a pending credit from this dropdown:<br />

<form action="credit_status_update_detail_eagle.php" method="post" name="form2" id="form2">
    <select name="mcred_order_num" >
        <option value="" selected="selected">Select a credit</option>
        <?php
            $query="SELECT mcred_order_num, mcred_memo_num,   mcred_acct_user_id  from memo_credits_temp WHERE  mcred_approbation NOT IN ( 'refuse','refused','approved','approve')  AND mcred_acct_user_id in (SELECT distinct  user_id  FROM `accounts` WHERE main_lab in(47,50) )   order by mcred_order_num";
