...
if ($_REQUEST[mcred_memo_num] <> ''){
	...
	$order_num      = substr($mcred_memo_num,1,5);
}else{
...
$DataCreditFinished    = mysqli_fetch_array($resultCreditFinished,MYSQLI_ASSOC);
...
<body>
  <table border="0" cellspacing="0" cellpadding="0" width="95%">
  	<tr valign="top">
  		...
  		<td width="1" bgcolor="#FFFFFF">&nbsp;</td>
  		<td width="75%">
<form  method="post" name="verification" id="verification" action="edit_credit_request_hbc.php">
...
$BalanceOptipoints =0;//Pas d'optipoints dans HBC			

$queryMemoCode         = "SELECT * FROM memo_codes WHERE mc_lab = $DataOrder[lab] and memo_code = '$DataCredit[mcred_memo_code]'";
$resultMemoCode        = mysqli_query($con,$queryMemoCode)		or die ('Could not check already credited b2 because: ' . mysqli_error($con));
...
<tr>
  ...
  <br><br>
   ...
   //echo '<a href="/admin/print_credit.php?memo_num="'. $mcred_memo_num.'>Print Credit </a>'; ?>
	...
	window.open( "/admin/print_credit_hbc.php?memo_num=<?php echo $mcred_memo_num?>" )
	</script>
	<?php } ?>
...
<table width="85%" border="1" cellpadding="2" cellspacing="0" class="formField">  
<form  method="post" name="edit_credit_request" id="edit_credit_request" method="post" action="edit_credit_request_hbc.php">                
...
<form  method="post" name="approve_credit" id="approve_credit" method="post" action="approve_credit_hbc.php">       
<input type="hidden" name="mcred_memo_num" id="mcred_memo_num" value="<?php echo $DataCredit[mcred_memo_num]; ?>" />
...
<form  method="post" name="refuse_credit" id="refuse_credit" method="post" action="refuse_credit_hbc.php">       
<input type="hidden" name="mcred_memo_num" id="mcred_memo_num" value="<?php echo $DataCredit[mcred_memo_num]; ?>" />
