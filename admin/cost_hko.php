...
if($_POST[rpt_search]=="Verify")
{
	...
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			WHERE prescript_lab=25 AND  orders.order_num IN ($_POST[order_num])";
			//echo '<br><br>'. $rptQuery;
			}else{
  ...
  <table border="0" cellspacing="0" cellpadding="0" width="95%">
  	<tr valign="top">
		...
		?></td>
  		...
  		<td width="1" bgcolor="#FFFFFF">&nbsp;</td>
  		<td width="75%">
<form  method="post" name="verification" id="verification" action="cost_hko.php">
...
<form  method="post" name="update_status" id="update_status" action="cost_hko.php">
			<?php 
			if ($rptQuery!=""){
...
<tr bgcolor=\"#000000\"></tr>";
	  echo "<tr>
				...
				<td align=\"center\"><strong>Cost Coating (US)</strong></td>
				<td align=\"center\"><strong>Edging Coating (US)</strong></td>
				<td align=\"center\"><strong>Tint (US)</strong></td>
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
			...
			$queryOrderFrom  = "SELECT order_product_id, order_from, order_product_name, prescript_lab, eye, order_product_coating FROM orders WHERE order_num =" . $listItem[order_num];
...
switch ($DataOrderFrom[order_from]){
	case 'ifcclubca':  	 $queryCost = "SELECT cost,cost_us, product_code, product_name, primary_key,coating FROM ifc_ca_exclusive WHERE primary_key = " . $DataOrderFrom[order_product_id]; break;
	case 'safety':       $queryCost = "SELECT cost,cost_us, product_code, product_name, primary_key,coating  FROM safety_exclusive WHERE primary_key = " . $DataOrderFrom[order_product_id]; break; 
	case 'directlens':   $queryCost = "SELECT cost,cost_us, product_code, product_name, primary_key ,coating FROM exclusive WHERE primary_key = " 		. $DataOrderFrom[order_product_id]; break; 
	case 'lensnetclub':  $queryCost = "SELECT cost,cost_us, product_code, product_name, primary_key,coating  FROM exclusive WHERE primary_key = " 		. $DataOrderFrom[order_product_id]; break; 
	case 'aitlensclub':  $queryCost = "SELECT cost,cost_us, product_code, product_name, primary_key,coating  FROM exclusive WHERE primary_key = " 		. $DataOrderFrom[order_product_id]; break; 
	case 'eye-recommend':$queryCost = "SELECT cost,cost_us, product_code, product_name, primary_key,coating  FROM exclusive WHERE primary_key = " 		. $DataOrderFrom[order_product_id]; break; 
	default: $queryCost = "" ; break; 
			...
			switch ($DataCost[coating]){
				case 'HD AR': 									$CostCoating='8.80';  break;
				case 'Dream AR': case 'AR+ETC': case 'ITO AR': 	$CostCoating='8.60';  break;
				case 'AR Backside': 							$CostCoating='3.00';  break;
				//ENTRER LES AUTRES CAS POSSIBLES..
			}//END SWITCH
			...
			if ($DataOrderFrom[eye] == 'Both'){
				$CostUS_RE = ($DataCost[cost_us]-$CostCoating)/2;
				$CostUS_RE = money_format('%.2n',$CostUS_RE);	
				
				$CostUS_LE = ($DataCost[cost_us]-$CostCoating)/2;
				$CostUS_LE = money_format('%.2n',$CostUS_LE);	
			}
	
	
	$CoutTaillage=0;//Prix par d�faut
	//V�rifier s'il y a eu du taillage fait par HKO
	...
	if (substr($listItem[order_product_name],0,5)=='K-ONE')
	{
		...
		if ($JobType =='Edge and Mount'){
			//HKO a fait le taillage, on doit afficher leur prix de taillage.
			$CoutTaillage = 2;	
		}//END IF
			 ...
			 echo "<td style=\"font-size:16px;\" align=\"center\">$listItem[order_num]</td>
				   ...
				   <td style=\"font-size:16px;\" align=\"center\">$CostCoating</td>
				   <td style=\"font-size:16px;\" align=\"center\">$CoutTaillage</td>
				   <td style=\"font-size:16px;\" align=\"center\">$ExtraTeinte</td>
