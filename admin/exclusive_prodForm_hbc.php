...
if (!$_POST[createProduct] == "Create Product"){
	$query="SELECT * from ifc_ca_exclusive WHERE primary_key = '$pkey'";
...
<link href="admin.css" rel="stylesheet" type="text/css" />
  ...
  <form name="form4" method="post" action="update_exclusive_product_hbc.php"  enctype="multipart/form-data">
    <table width="100%" border="0" cellpadding="2" cellspacing="0">
      ...
      <tr bgcolor="#DDDDDD">
        <td width="16%" align="right" nowrap="nowrap"><p> <font size="1" face="Arial, Helvetica, sans-serif">Product Name:</font></p></td>
	...
	<?php  
	$queryCollection = "SELECT collection from ifc_ca_exclusive WHERE primary_key = $productData[primary_key]";
	$resultCollection=mysqli_query($con,$queryCollection)		or die ("Could not select items");
	$DataCollection=mysqli_fetch_array($resultCollection,MYSQLI_ASSOC);
	$Collection = $DataCollection[collection]; 
	?>

	<select name="collection">
	<option  <?php if ($Collection == '') echo 'selected'; ?> value="">Select a collection</option>
		
	<option  <?php if ($Collection == 'HBC STC') 	 echo 'selected'; ?> value="HBC STC"  >HBC STC</option>
	<option  <?php if ($Collection == 'HBC SURFACE') echo 'selected'; ?> value="HBC SURFACE" >HBC SURFACE</option>
	<option  <?php if ($Collection == 'HBC STOCK') echo 'selected'; ?> value="HBC STOCK" >HBC STOCK</option>
	<option  <?php if ($Collection == 'HBC SWISS') 	 echo 'selected'; ?> value="HBC SWISS">HBC SWISS</option>
	</select>	
</td>
      </tr>
...
<?php  
$queryLens = "Select lens_category from ifc_ca_exclusive where primary_key = $productData[primary_key]";
...
<option  <?php if ($lenscatego == 'sv') echo 'selected'; ?> value="sv">SVv</option>
...
<option  <?php if ($lenscatego == '') echo 'selected'; ?> value="">None</option>
      ...
      <tr bgcolor="#DDDDDD">
        ...
        <td align="right"><font size="1" face="Arial, Helvetica, sans-serif"><strong>COST US</strong>
          :</font></td>
        <td bgcolor="#ED8E90" align="left"><font size="1">
          <input name="cost_us" type="text" class="formText" id="cost_us" value="<?php echo "$productData[cost_us]"; ?>" size="6" />
        </font></td>
  ...
  <?php
  $query="select coating from ifc_ca_exclusive group by coating asc"; /* select all openings */
