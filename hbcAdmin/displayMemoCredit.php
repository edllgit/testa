...
if($_POST[issueMemo]=="Issue Memo"){
	...
	if($memo_test==true){
		...
		$heading="MEMO CREDIT IS NOW PENDING APPROVAL, AN EMAIL WILL CONFIRM";
		$pendingApproval = "yes";
...
$memoQuery="SELECT * from memo_credits
			LEFT JOIN (orders) ON (memo_credits.mcred_order_num = orders.order_num) 
			LEFT JOIN (memo_codes) ON (memo_credits.mcred_memo_code = memo_codes.memo_code) 
			WHERE memo_credits.mcred_memo_num='$mcred_memo_num' AND memo_codes.mc_lab = '$lab_pkey'"; 
		...
		?></td>
  		...
  		<td width="75%">
            	...
            	<tr bgcolor="#000000">
					...
					echo "MEMO ORDER"; 
           			...
           			<td width="33%" align="center" nowrap><form name="formPrint" id="formPrint" method="post" action="displayMemoCreditForm.php" target="_blank">
                        ...
                        <input name="mcred_detail" type="hidden" id="mcred_detail" value="<?php echo $memoData[mcred_detail];?>">
       				<input <?php if ($pendingApproval == "yes") echo ' disabled '; ?> type="submit"  name="printMemo" value="Print Memo" class="formField"></form></td>
            	</tr>
			<tr><td colspan="3">
			...
			<tr bgcolor="#DDDDDD">
				...
				<td align="right" valign="middle" nowrap>Detail</td>
				<td align="left" valign="middle"><?php echo  $memoData[mcred_detail]; ?></td>
			</tr>
			<tr bgcolor="#FFFFFF">
				<td colspan="4" align="center" valign="middle"><input <?php if ($pendingApproval == "yes") echo ' disabled '; ?> type="submit" name="emailMemo" value="Email Memo to Customer" class="formField">
				&nbsp;&nbsp;&nbsp;
					<input type="submit" name="backToMain" <?php if ($pendingApproval == "yes") echo ' disabled '; ?> value="Cancel" class="formField"></td>
				</tr>
			</table>
			</form></td>
  </tr>
...
<?php if ($pendingApproval == "yes") echo ' Due to new changes, Memo credits now need to be approved by an administrator. Your credit request is now pending approval. An email will confirm once the status will be updated. Thanks '; ?>
 &nbsp;<br>
</td>
	  </tr>
</table>
