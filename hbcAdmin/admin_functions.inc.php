...
//credit limit check
	...
	if (($limitcount > 0) && ((!$_POST["cl_limit_amt"]) || ($_POST["cl_limit_amt"] == 0))) {
		...
		$limitResult=mysqli_query($con,$limitQuery)
			or die ("Could not delete credit limit because ".mysqli_error($con));
	}
...
function make_the_pmt()
{
	$result=mysqli_query($con,"SELECT curdate()");/* get today's date */
		//$today=mysqli_result($result,0,0);
	$pmt_type="check";
...
$send_to_address=array('dbeaulieu@direct-lens.com','dbeaulieu@direct-lens.com');
$curTime= date("m-d-Y");	
