...
if($_POST[rpt_search]=="Verify")
{
	...
	$Array_OrderNum = array_filter(array_map('trim', $Array_OrderNum));
		...
		$valueSansEspace = trim($value, " ");
			...
			AND accounts.user_ID IN $User_ID_IN  GROUP BY order_num ORDER BY orders.order_num";
			//echo '<br><br>'. $rptQuery;
			}else{
...
//Impression par Batch-->Demande d'Halifax Juin 2017
 if($_POST[rpt_search_batch]=="Verify")
{
	$PassValidation = true;
...
<form  method="post" name="verification" id="verification" action="hbc_fast_print.php">
...
<form  method="post" name="update_status" id="update_status" action="hbc_fast_print.php">
			<?php 
			if ($rptQuery!=""){
...
while ($listItem=mysqli_fetch_array($rptResult,MYSQLI_ASSOC)){
			...
			if (($list_order_status=='Cancelled')||($list_order_status=='Shipped')){
 ...
 if ($OrderAbleToUpdate > 0){
	...
	<td align=\"center\" colspan=\"11\" nowrap=\"nowrap\">";
				  ...
				  if ($OrderAbleToUpdate > 0){
				  echo "<input name=\"update_status\" style=\"font-size:14px;\"  type=\"submit\" id=\"rpt_search\" value=\"Print selected orders\" class=\"formField\"></tr>";
				  }else{
				  echo "<input name=\"update_status\" style=\"font-size:14px;\"  type=\"submit\" id=\"rpt_search\" disabled=\"disabled\" value=\"Print selected orders\" class=\"formField\"></tr>";
				  }
...
All these orders are either cancelled or shipped, so their cannot be added on the Report: frames sent to the lab.</div>';
?>
