<table width="100%" border="0" align="center" cellpadding="3" cellspacing="0"  class="formField3">
         <tr>
			  ...
			  $resultOF=mysqli_query($con,$queryOF)			or die  ('I cannot select items because 654: ' . mysqli_error($con));
			...
			$resultMirror = mysqli_query($con,$queryMirror)			or die  ('I cannot select items because 6555: ' . mysqli_error($con));
			...
			$DetailMirror = 'Mirror:  ' . $DataMirror[tint_color];
			  ...
			  if (($OrderFrom =='ifcclub') || ($OrderFrom =='ifcclubca')){
				  ...
				  $resultPackage = mysqli_query($con,$queryPackage)			or die  ('I cannot select items because 1175: ' . mysqli_error($con));
           ...
           <td><h3>EYE:&nbsp;&nbsp;&nbsp;<b><?php echo $listItem[eye]; ?></b></h3></td>
              ...
              <tr >
			   ...
			   $resultWarranty = mysqli_query($con,$queryWarranty)	or die ("Could not select items g78t422");
			   $DataWarranty   = mysqli_fetch_array($resultWarranty,MYSQLI_ASSOC);
			   ?>
               
               
			   <?php if (($leUserID!="88433") && ($leUserID!="88438") && ($leUserID!="88439"))
			   { ?>
			   <td rowspan="7" align="right" valign="top"><b>$
			   <?php echo $listItem[order_product_price]; 
			   }
			   ?></b></td>
				...
				$resultDescriptionCoupon=mysqli_query($con,$queryDescriptionCoupon)	or die ("Could not select items F433S". mysqli_error($con));
			 ...
			 $resultTopUrgent=mysqli_query($con,$queryTopUrgent)	or die ("Could not select items F2398");
			 ...
			 $resultTopUrgent=mysqli_query($con,$queryTopUrgent)	or die ("Could not select items ijh54ou43");
			 $countTopUrgent=mysqli_num_rows($resultTopUrgent);
			 $DataUrgent=mysqli_fetch_array($resultTopUrgent,MYSQLI_ASSOC);
			 if  ($countTopUrgent > 0)
			...
			$queryMirror = "select * FROM extra_product_orders WHERE category = 'Mirror' AND order_num =  " . $listItem[order_num];
			$resultMirror=mysqli_query($con,$queryMirror)	or die ("Could not select items gg5d");
			...
			$resultMirror=mysqli_query($con,$queryMirror)	or die ("Could not select items fdsw67");
			$countMirror=mysqli_num_rows($resultMirror);
			$DataMirror=mysqli_fetch_array($resultMirror,MYSQLI_ASSOC);
			if  ($countMirror > 0)
			...
			$resultMirror=mysqli_query($con,$queryMirror)	or die ("Could not select items  3245s3");
			$countMirror=mysqli_num_rows($resultMirror);
			$DataMirror=mysqli_fetch_array($resultMirror,MYSQLI_ASSOC);
			if  ($countMirror > 0)
			...
			$resultMirror=mysqli_query($con,$queryMirror)	or die ("Could not select items F432");
			$countMirror=mysqli_num_rows($resultMirror); 
			$DataMirror=mysqli_fetch_array($resultMirror,MYSQLI_ASSOC);
			if  ($countMirror > 0)
