...
if ($_POST[from_account_update]=="true"){//switch the user_id of the  order 
	...
	//echo $queryOldUserid;
	$resultOldUserID = mysqli_query($con,$queryOldUserid)	or die ("Could not select items 122");
				 ...
				 echo 'disabled="disabled"';
				  ?>  name="order_status" <?php if ($update_status <> 'yes') echo 'disabled="disabled"'; ?> class="formField3">
                      ...
                      <?php if ($mylang == 'lang_french') {?>Attente de monture<?php }else{ ?>Waiting for Frame<?php } ?>
					   ...
					   <option value="waiting for frame store"<?php if(strtolower($orderData[order_status])=="waiting for frame store") echo " selected";?>>
                      <?php if ($mylang == 'lang_french') {?>Attente de monture Magasin<?php }else{ ?>Waiting for Frame Store<?php } ?>
                      </option>
					  
					   <option value="waiting for frame ho/supplier"<?php if(strtolower($orderData[order_status])=="waiting for frame he/supplier") echo " selected";?>>
                      <?php if ($mylang == 'lang_french') {?>Attente de monture Siege Social/Fournisseur<?php }else{ ?>Waiting for Frame Head Office/Supplier<?php } ?>
                      </option>
                      ...
                      <?php if ($mylang == 'lang_french') {?>En attente de monture Swiss<?php }else{ ?>Waiting for Frame Swiss<?php } ?>
                      </option>
					  ...
					  <option value="scanned shape to swiss"<?php if(strtolower($orderData[order_status])=="scanned shape to swiss") echo " selected";?>>
                        <?php if ($mylang == 'lang_french') {?>Trace envoyé a Swiss<?php }else{ ?>Scanned shape to Swiss<?php } ?>
                      </option>
...
?>
			...
			$orderResult2=mysqli_query($con,$orderQuery2)		or die  ('I cannot select items because: ' . mysqli_error($con));
			$orderData2=mysqli_fetch_array($orderResult2,MYSQLI_ASSOC);
			
			
            $query="SELECT primary_key, company, last_name, first_name, user_id, product_line from accounts where main_lab='$_SESSION[lab_pkey]' AND approved='approved'
			AND user_id not in (88409,88405,88449,88403,88408,88411,88414,88440,88442,88431,88432,88434,88435,88441,88429,88416,88438,88439,88444,88430,88433,88666) ORDER BY product_line, company, last_name";
            //echo '<br>'. $query;
...
$itemSubtotal=$listItem[order_quantity]*($listItem[order_product_price]+$e_total_price)+$over_range-$coupon_dsc;
	...
	<div class="formField3">
		<a href="/hbcAdmin/report.php"><?php echo $adm_backtoorderlist_txt;?></a>
	</div></td>
  </tr>
</table>
 &nbsp;<br>
