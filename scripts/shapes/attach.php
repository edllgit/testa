...
[ ]	5-Flagger ce transfert  dans shape_copied_ftp.*/
	...
	$nomFichier = $NomdeLaFormeAtrouver;
	...
	//=====================================
	$repertoireSource = "../../../../../../ftp_root/Banque de traces";

	// Liste des fichiers dans le répertoire source
	$fichiers = scandir($repertoireSource);
	
	// Parcourir chaque fichier pour trouver le correspondant au nom recherché
foreach ($fichiers as $fichier) {
    ...
    if ($fichier == '.' || $fichier == '..') {
	    ...
	    // Ignorer les espaces et les tirets dans le nom de fichier
    $nomFichierSansEspaces = str_replace(' ', '', $fichier);
    $nomFichierSansTirets = str_replace('-', '', $fichier);
    $nomFichierSansEspacesTirets = str_replace([' ', '-'], '', $fichier);
	
	   // Vérifier si le fichier correspond au nom recherché (ignorant espaces et tirets)
    if (strcasecmp($nomFichier, $nomFichierSansEspaces) === 0 || 
        strcasecmp($nomFichier, $nomFichierSansTirets) === 0 || 
        strcasecmp($nomFichier, $nomFichierSansEspacesTirets) === 0) {
        // Chemin complet du fichier trouvé
        $cheminFichierSource = $repertoireSource . '/' . $fichier;
		...
		$repertoireDestination = "FROM_DL/shapes";
		        // Copier le fichier dans le répertoire de destination
        $cheminFichierDestination = $repertoireDestination . '/' . $fichier;
        if (copy($cheminFichierSource, $cheminFichierDestination)) {
            echo "Le fichier $fichier a été copié avec succès.\n";
        } else {
            echo "Erreur lors de la copie du fichier $fichier.\n";
        }
	...
	if ($PositionFormedansArray!=true){
