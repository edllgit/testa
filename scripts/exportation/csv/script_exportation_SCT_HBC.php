...
<head>
	...
	<!-- Bootstrap core CSS -->
    <link href="<?php constant('DIRECT_LENS_URL'); ?>/bootstrap/css/bootstrap.min.css" rel="stylesheet">
...
$filename = "SCT_HBC_OrderData-".$today.".csv";
$fp       = fopen($filename, "c");
$orderQuery="SELECT distinct orders.* from orders
WHERE  orders.prescript_lab in (3) 
...
while ($orderData = mysqli_fetch_array($orderResult,MYSQLI_ASSOC)){
	...
	$outputstring = export_order_DLAB_HBC($orderData[order_num]);
...
$file        = "SCT_HBC_OrderData-".$today.".csv";
$remote_file = "SCT_HBC_OrderData-".$today.".csv";
