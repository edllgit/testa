...
echo '<br> constant4 : '.var_dump (__DIR__."../../../export_functions.inc.php");
...
$orderQuery="SELECT * from orders 	WHERE order_status='processing' AND  order_product_type='exclusive' AND prescript_lab = 77    ORDER by order_num";
...
$ftp_server = constant("PROCREA_FTP");
echo '<br> constant4 : '.var_dump ($ftp_server);
$ftp_user = constant("FTP_USER_PROCREA");

$ftp_pass = constant("FTP_PASSWORD_PROCREA");
...
if (@ftp_login($conn_id, $ftp_user, $ftp_pass)) {
	...
	//echo '<br> constant5 : '.var_dump ($ftp_user@$ftp_server);
} else {
    echo "Couldn't connect as $ftp_user\n";
	//echo '<br> constant6 : '.var_dump ($ftp_user);
}
...
if (ftp_put($conn_id, $remote_file, $file, FTP_BINARY)) {
 ...
 //echo "Erreur FTP : " . ftp_error($conn_id);
