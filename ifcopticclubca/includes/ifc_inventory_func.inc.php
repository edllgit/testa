...
$message  .= '<br><br>'.$sql; 
$result	   = mysqli_query($con,$sql)	or die  ('ERROR ' . mysqli_error($con).' 1sql='.$sql);
$epItems   = mysqli_fetch_assoc($result,MYSQLI_ASSOC);
...
if ($itemCount > 0 ){//A FRAME HAS BEEN ORDERED
	...
	$message     .= '<br><br>'.$sql; 
	$result		  = mysqli_query($con,$sql) or die  ('ERROR ' . mysqli_error($con).' 2sql='.$sql);
	...
	$result      = mysqli_query($con,$sql)			or die  ('ERROR ' . mysqli_error($con).' 3sql='.$sql);
	$nbrResultat = mysqli_num_rows($result);
	...
	if ($nbrResultat > 0)//SI > 0, le tuple existe dans product_inventory_ifc ON FAIT DONC UNE MISE A JOUR
	{
		...
		/*if ($new_inventory == 0){//Si l'inventaire est mit a 0 pour ce produit..on envoie un email
			...
			$resultDelete = mysqli_query($con,$queryDelete)	or die  ('ERROR ' . mysqli_error($con).' 5sql='.$sql);
			$DeletedFromInv = 'yes';
		}*/
		...
		//MISE A JOUR DE L'INVENTAIRE
			...
			$result=mysqli_query($con,$sql)	or die  ('ERROR ' . mysqli_error($con).' 4sql='.$sql);
...
include "../sec_connectEDLL.inc.php";
		...
		$resultFrameA = mysqli_query($con,$queryFrameA)			or die  ('ERROR ' . mysqli_error($con).' sql='.$queryFrameA);
		$DataFrameA   = mysqli_fetch_assoc($resultFrameA,MYSQLI_ASSOC);
