...
<table width="770" border="0" align="center" cellpadding="3" cellspacing="0"  class="formBox">
	   ...
	   <?php echo 'deb0'.var_dump($LabNum).$queryLab;?> 
       
      <?php if (($LabNum==66) ||($LabNum==67)){   ?>
       <?php echo 'deb = main_lab = 66 || 67';?>
      <form id="<?php echo  $listItem[order_item_number] ?>" name="<?php echo  $listItem[order_item_number] ?>" method="post" action="basket.php" >
           ...
           <td bgcolor="#D5EEF7" colspan="8"> <input name="Valider" type="submit" class="formText" value="<?php echo 'Valider cette commande';?>" 
		   <?php if ($listItem[coupon_dsc]<> 0.01) { echo 'type="submit"'; } else { echo 'type="hidden"'; } ?>/>
		   </td>
      ...
      <?php }  ?>
         ...
         <tr >
           ...
           <?php  if($listItem[myupload] != ""){
           	echo "The file ".$listItem[myupload]." has been uploaded as a lens profile .".var_dump($listItem[coupon_dsc]);
...
<input name="pkey" type="hidden" value="<?php echo  $listItem[primary_key]?>" /></form><?php */?></td>
			...
			$FRAME_result=mysqli_query($con,$FRAME_query)					or die  ('I cannot select items because: ' . mysqli_error($con));
			$Frame_item=mysqli_fetch_array($FRAME_result,MYSQLI_ASSOC);
			...
			$Edging_result= mysqli_query($con,$Edging_query)				or die  ('I cannot select items because: ' . mysqli_error($con));
			$Data_Edging  = mysqli_fetch_array($Edging_result,MYSQLI_ASSOC);
			...
			$MIRROR_result = mysqli_query($con,$MIRROR_query)					or die  ('I cannot select items because: ' . mysqli_error($con));
			$MIRROR_item   = mysqli_fetch_array($MIRROR_result,MYSQLI_ASSOC);
			...
			$resultOverRange =mysqli_query($con,$QueryOverRange)					or die  ('I cannot select items because: ' . mysqli_error($con));
			$DataOverRange=mysqli_fetch_array($resultOverRange,MYSQLI_ASSOC);
			...
			$ResultEdgePolish    = mysqli_query($con,$queryEdgePolish)					or die  ('I cannot select items because: ' . mysqli_error($con));
			$DataEdgePolish = mysqli_fetch_array($ResultEdgePolish,MYSQLI_ASSOC);
			$EdgePolish  = $DataEdgePolish[price];
					
			//IIMPACT
			$queryIIMPACT     = "SELECT price from extra_product_orders  WHERE  order_id='$listItem[primary_key]' AND category='Iimpact' ";
			//echo '<br>'. $queryIIMPACT;
			...
			$IIMPACT  		  = $DataIIMPACT[price];		
				...
				if (($EdgePolish<> '') &&  ($mylang == 'lang_french')){echo  "<br> Edge Polish: ";echo   $EdgePolish . '$' ;}
					
					
				if (($IIMPACT<> '') &&  ($mylang == 'lang_english')){echo  "<br> Iimpact: ";echo  '$' . $IIMPACT ;}	
				if (($IIMPACT<> '') &&  ($mylang == 'lang_french')){echo  "<br> Iimpact: ";echo   $IIMPACT . '$' ;}		
